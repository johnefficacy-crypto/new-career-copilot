-- Migration 038: aggregator candidate layer for runtime.
--
-- Brings four tables from legacy migration 044 into the active set so
-- the scraper runtime can adopt the
--   discovery -> candidate merge -> official resolver -> extraction
-- pipeline described in the scraping audit. Tables are non-breaking
-- (the existing scrape_queue path keeps working); the runner starts
-- writing into them as part of the same PR.
...
create table if not exists public.aggregator_listings (
  ...
  event_type           text not null default 'new_recruitment',
...
create index if not exists idx_aggregator_listings_status_seen
  on public.aggregator_listings(status, last_seen_at desc);

create index if not exists idx_aggregator_listings_run
  on public.aggregator_listings(scrape_run_id);
