...
def test_high_duplicate_blocks_without_confirmation(monkeypatch):
    class SB8(SB):
        def table(self,t):
            ...
            return super().table(t)
    import pytest
    sb=SB8(); sb.state['queue'][0].update({'status':'approved','source_id':'s1','official_source_resolved':True,'extraction_status':'verified','reviewer_notes':''}); monkeypatch.setattr(admin_scrape,'get_supabase_admin',lambda:sb)
    with pytest.raises(Exception):
        admin_scrape.promote_queue_item('q1', {'id':'a','email':'e'})
