...
import { CheckboxField, InputField, LoadingSkeleton, SelectField } from "../shared/ui";
...
export default function NotificationPreferences() {
  ...
  if (!prefs) return <LoadingSkeleton variant="form" className="max-w-3xl" />;
  ...
  return <div className="space-y-4"><h1 className="font-heading text-3xl">Notification preferences</h1>
    ...
    <div className="soft-card rounded-2xl p-4 grid md:grid-cols-3 gap-3">
      ...
      <SelectField label="Min priority" value={prefs.min_priority_in_app||'low'} onChange={(e)=>setPrefs({...prefs,min_priority_in_app:e.target.value})}><option>low</option><option>medium</option><option>high</option></SelectField>
      <SelectField label="Deadline reminders" multiple value={prefs.deadline_reminder_windows||[]} onChange={(e)=>setPrefs({...prefs,deadline_reminder_windows:Array.from(e.target.selectedOptions).map(o=>o.value)})}><option value="48h">48h</option><option value="24h">24h</option><option value="6h">6h</option></SelectField>
      <InputField label="Quiet start hour" type="number" min="0" max="23" value={prefs.quiet_hours_start ?? ''} onChange={(e)=>setPrefs({...prefs,quiet_hours_start:e.target.value===''?null:Number(e.target.value)})} />
      <InputField label="Quiet end hour" type="number" min="0" max="23" value={prefs.quiet_hours_end ?? ''} onChange={(e)=>setPrefs({...prefs,quiet_hours_end:e.target.value===''?null:Number(e.target.value)})} />
    </div>
