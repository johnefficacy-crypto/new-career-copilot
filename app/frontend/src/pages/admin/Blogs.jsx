...
export default function AdminBlogs() {
  ...
  const [missingCta, setMissingCta] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    if (missingSeo) params.set("missing_seo", "true");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (editingId) await api.put(`/api/admin/blogs/${editingId}`, form);
      else await api.post("/api/admin/blogs", form);
      setForm(EMPTY_FORM); setEditingId(null); await load();
    } catch (err) { setError(getApiErrorMessage(err)); }
    finally { setSaving(false); }
  };

  const startEdit = (x) => { setEditingId(x.id); setForm({ ...EMPTY_FORM, ...x }); };
  const previewUrl = editingId ? `/blog/${form.slug}` : null;
  ...
  return <div className="stack" style={{ gap: 16 }}>
    <section className="card p-4">
      ...
      <div className="row" style={{ gap: 8, marginTop: 12 }}>
        ...
        <label className="text-xs"><input type="checkbox" checked={missingSeo} onChange={(e) => setMissingSeo(e.target.checked)} /> missing SEO</label>
        <label className="text-xs"><input type="checkbox" checked={missingCta} onChange={(e) => setMissingCta(e.target.checked)} /> missing CTA</label>
    ...
    <section className="card p-4">
      <h3 className="font-semibold">{editingId ? `Edit blog #${editingId}` : "Create blog"}</h3>
      <form onSubmit={onSubmit} className="stack" style={{ gap: 8 }}>
        ...
        <input className="input" placeholder="Secondary CTA label" value={form.secondary_cta_label || ""} onChange={(e) => setForm({ ...form, secondary_cta_label: e.target.value })} />
        <input className="input" placeholder="Secondary CTA URL" value={form.secondary_cta_url || ""} onChange={(e) => setForm({ ...form, secondary_cta_url: e.target.value })} />
        ...
        <div className="row" style={{ gap: 8 }}>
          <button className="btn" disabled={saving} type="submit">{saving ? "Saving..." : editingId ? "Update blog" : "Create blog"}</button>
          {editingId ? <button className="btn ghost" type="button" onClick={() => { setEditingId(null); setForm(EMPTY_FORM); }}>Cancel edit</button> : null}
          {previewUrl ? <a className="btn ghost" href={previewUrl} target="_blank" rel="noreferrer">Preview</a> : null}
        </div>
      </form>
    </section>
    ...
    <section className="card p-4">
      ...
      <div className="stack" style={{ gap: 6 }}>
        {items.map((x) => <div key={x.id} className="border rounded p-2 text-sm">
          ...
          <div className="text-xs text-muted-foreground mb-2">/{x.slug} · {x.status} · intent {x.primary_intent || "-"}</div>
          <div className="row" style={{ gap: 8 }}>
            <button className="btn ghost" type="button" onClick={() => startEdit(x)}>Edit</button>
            <button className="btn ghost" type="button" onClick={async () => { await api.post(`/api/admin/blogs/${x.id}/publish`, {}); await load(); }}>Publish</button>
            <button className="btn ghost" type="button" onClick={async () => { await api.post(`/api/admin/blogs/${x.id}/archive`, {}); await load(); }}>Archive</button>
          </div>
        </div>)}
      </div>
    </section>
