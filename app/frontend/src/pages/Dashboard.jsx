...
function MatchSection({ title, items }) {
  return (
    <div className="soft-card rounded-2xl p-4">
      ...
      {items.length === 0 ? <div className="mt-2 text-sm text-muted-foreground">No recommendations in this stage yet.</div> : <ul className="mt-2 space-y-2">{items.map((m) => <li key={`${title}-${m.id || m.slug}`} className="text-sm"><Link to={`/app/exams/${m.slug}`} className="font-medium link-under">{m.name}</Link><div className="text-xs text-muted-foreground">Score {m.match_score} · Next: {m.next_action}</div></li>)}</ul>}
    </div>
  );
}
...
export default function Dashboard() {
  const auth = useAuth();
  const { recommendations, recommendationsAvailable, recruitments, plan, focus, review, apps, profileCompletion, loading, errors } = useDashboardData();
  ...
  const rankedFallbackMatches = useMemo(() => rankRecruitments(recruitments.items, auth.user, { appByRecruitmentId, backlogHigh, studyHoursWeek: focus.total_hours_7d }), [recruitments.items, auth.user, appByRecruitmentId, backlogHigh, focus.total_hours_7d]);
  ...
  const stageSections = useMemo(() => ({ apply_now: topMatches.filter((m) => m.recommendation_stage === "apply_now").slice(0, 3), continue_application: topMatches.filter((m) => m.recommendation_stage === "continue_application").slice(0, 3), prepare_after_submission: topMatches.filter((m) => m.recommendation_stage === "prepare_after_submission").slice(0, 3), complete_profile: topMatches.filter((m) => m.recommendation_stage === "complete_profile").slice(0, 3) }), [topMatches]);
  ...
  return (
    <div data-testid="dashboard-page" className="space-y-6">
      ...
      <TodaysActions topMatches={topMatches} pendingDocs={pendingDocs} inProgressForms={inProgressForms} backlogHigh={backlogHigh} profileCompletion={profileCompletion} />

      {(errors.recruitments || errors.plan || errors.focus || errors.review || errors.apps || errors.profileCompletion) && <ErrorState title="Some dashboard modules failed to load" message="You can still continue with available data. Refresh to retry." />}
