  ...
  return (
    <div className="min-h-screen flex paper-bg">
      ...
      <div className="flex-1 min-w-0">
        <TopBar className="bg-[#FBF6EF]/80 backdrop-blur" left={<button className="lg:hidden h-9 w-9 grid place-items-center rounded-lg border border-border" onClick={() => setSidebarOpen(true)} data-testid="mobile-menu-toggle"><Menu className="h-4 w-4" /></button>} center={<div className="relative flex-1 max-w-xl"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><input data-testid="global-search" placeholder="Search exams, threads, mentors…" className="w-full pl-9 pr-4 py-2 rounded-lg bg-white/80 border border-border focus:border-clay-400 text-sm outline-none" /></div>} right={<><Link to="/app/notifications" data-testid="notif-btn" className="h-9 w-9 grid place-items-center rounded-lg border border-border bg-white/70 relative"><Bell className="h-4 w-4" />{unreadCount > 0 && <span className="absolute -top-1 -right-1 text-[10px] leading-none px-1.5 py-0.5 rounded-full bg-clay-500 text-white">{unreadCount > 9 ? "9+" : unreadCount}</span>}</Link><Link to="/app/profile" className="h-9 w-9 grid place-items-center rounded-lg border border-border bg-white/70" data-testid="settings-btn"><Settings className="h-4 w-4" /></Link><UserMenu user={auth.user} onLogout={auth.logout} /></>} />
