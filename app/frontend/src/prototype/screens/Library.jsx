...
import { CART, LIBRARY, PRODUCTS, RECENTLY_VIEWED, SELLERS } from "../data/market";
import {
  Card, EmptyState, Eyebrow, FooterStrip, KPI, PageHeader, Pill, PriceBlock,
  ProductCover, PrototypePage, SellerTrustBadge, Tabs,
} from "../ui";
    ...
    <div className="rounded-xl border border-[#E7DECB] bg-white/70 p-4 flex gap-4 items-start">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {p.planRelevant ? <Pill tone="sage" className="!text-[9.5px]">◐ For your plan</Pill> : null}
          {completed ? <Pill tone="ink">Completed</Pill> : null}
          {item.expiringIn ? <Pill tone="amber">Access expires in {item.expiringIn}</Pill> : null}
        </div>
        ...
        <div className="mt-3 flex items-center gap-3">
          <div className="flex-1 h-[6px] bg-[#EFE2C9] rounded-full overflow-hidden">
            <div className="h-full bg-[#54794E]" style={{ width: `${Math.round(item.progress * 100)}%` }} />
          </div>
          <span className="num-mono text-[11px] text-clay-700">
            {Math.round(item.progress * 100)}% · last used {item.lastUsedAt}
          </span>
        </div>
        <div className="mt-3 flex flex-wrap gap-1.5">
          <button className="text-[11.5px] px-3 py-1.5 rounded-full bg-[#2E2218] text-[#F3EADB] font-semibold">Open</button>
          <button className="text-[11.5px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Download</button>
          <button className="text-[11.5px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Receipt</button>
          {item.progress < 0.25 && p.refundDays ? (
            <button className="text-[11.5px] px-3 py-1.5 rounded-full border border-[#D9B4A6] text-[#7A3925] font-semibold">Request refund</button>
          ) : null}
...
function LibraryView() {
  ...
  const avgProgress = active.length
    ? Math.round((active.reduce((a, b) => a + b.progress, 0) / active.length) * 100)
    : 0;
  return (
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <KPI k="Avg progress" v={`${avgProgress}%`} tone="sage" sub="across active" />
      ...
      <Card padded={false}>
        ...
        <div className="px-7 pb-6 space-y-3">
          {active.map((item) => (
            <LibraryRow key={item.orderId} item={item} />
          ))}
        </div>
      </Card>
      {completed.length > 0 ? (
        <Card padded={false}>
          ...
          <div className="px-7 pb-6 space-y-3">
            {completed.map((item) => (
              <LibraryRow key={item.orderId} item={item} completed />
            ))}
          </div>
        </Card>
      ) : null}
...
function CheckoutSummary({ subtotal, tax }) {
  return (
    <Card className="lg:sticky lg:top-4">
      <Eyebrow>Order summary</Eyebrow>
      <h3 className="font-serif text-[20px] mt-1">Final price · no surprises.</h3>
      <ul className="mt-3 space-y-1.5 text-[13px]">
        <li className="flex justify-between"><span className="text-clay-700">Subtotal</span><span className="num-mono">₹{subtotal.toLocaleString()}</span></li>
        <li className="flex justify-between"><span className="text-clay-700">GST · included</span><span className="num-mono">₹{tax.toLocaleString()}</span></li>
        <li className="flex justify-between"><span className="text-clay-700">Platform fee</span><span className="num-mono text-[#33482F]">₹0</span></li>
      </ul>
      <div className="rule mt-3 pt-3 flex justify-between items-baseline">
        <span className="text-[13px]">Total</span>
        <span className="font-serif text-[24px] num-mono">₹{subtotal.toLocaleString()}</span>
      </div>
      ...
      <div className="num-mono text-[10.5px] text-clay-700 mt-2 text-center">UPI · Card · NetBanking · Wallets</div>
    ...
    <div className="grid lg:grid-cols-[1fr_360px] gap-6 items-start">
      <Card padded={false}>
        <div className="px-7 pt-6 pb-3 flex items-end justify-between gap-3 flex-wrap">
          ...
          <button className="text-[11.5px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Save all for later</button>
        </div>
        <div className="px-7 pb-6 space-y-3">
          {items.map((c, i) => (
            <div key={i} className="rounded-xl border border-[#E7DECB] bg-white/70 p-4 flex gap-4 items-start">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="num-mono text-[10.5px] text-clay-700 mt-1">{c.p.exam} · {c.p.lessonsLabel || c.p.pages}</div>
                <div className="mt-2"><PriceBlock p={c.p} /></div>
              ...
              <div className="text-right shrink-0">
                ...
                <div className="num-mono text-[10.5px] text-clay-700 mt-1">qty {c.qty}</div>
                <div className="mt-2 flex flex-col gap-1">
                  <button className="text-[10.5px] px-2 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Save</button>
          ...
          {items.length === 0 ? (
            <EmptyState icon="◌" title="Your cart is empty." body="Browse the marketplace for plan-relevant products." />
          ) : null}
        </div>
      </Card>
      <CheckoutSummary subtotal={subtotal} tax={tax} />
    </div>
  );
}
  ...
  return (
    <Card padded={false}>
      ...
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>When</th><th>Order ID</th><th>Product</th><th>Seller</th><th>Amount</th><th>Status</th><th className="right">Receipt</th></tr>
          </thead>
          <tbody>
            {LIBRARY.map((o) => {
              ...
              return (
                <tr key={o.orderId}>
                  <td className="num-mono text-clay-700">{o.purchasedAt}</td>
                  <td className="num-mono">{o.orderId}</td>
                  <td><strong>{p?.title}</strong></td>
                  <td>{seller?.name}</td>
                  <td className="num-mono">₹{o.price.toLocaleString()}</td>
                  <td><Pill tone="sage">paid</Pill></td>
                  <td className="right"><button className="text-[11px] text-clay-700 hover:text-clay-900 underline">Download invoice</button></td>
                </tr>
              );
            })}
          </tbody>
...
function SavedView() {
  return (
    <Card padded={false}>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 px-7 pb-6">
          ...
          return (
            <article key={id} className="rounded-xl border border-[#E7DECB] bg-white/70 p-4">
              ...
              <div className="mt-3 flex gap-1.5">
                ...
                <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Remove</button>
              </div>
            </article>
          );
        })}
...
export default function PrototypeLibrary() {
  ...
  return (
    <PrototypePage label="My library">
      <div className="px-10 pt-9">
        <PageHeader
          ...
          right={<a href="/app/marketplace" className="text-[12px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">← Back to marketplace</a>}
        />
      </div>
      <div className="px-10 space-y-6">
        ...
        {tab === "cart" ? <CartView /> : null}
        {tab === "orders" ? <OrdersView /> : null}
        {tab === "saved" ? <SavedView /> : null}
      </div>
      <FooterStrip />
    </PrototypePage>
  );
