...
import { ADMIN_MARKET, SELLERS } from "../data/market";
import {
  Avatar, Card, Eyebrow, FooterStrip, KPI, PageHeader, Pill, PrototypePage,
  SectionHeader, SellerTrustBadge, Tabs, TrustStamp,
} from "../ui";
...
function AdminMarketOverview() {
  return (
    <>
      <Card>
        <SectionHeader eyebrow="Today" title="What needs your attention." sub="High-severity first." />
          ...
          <ul className="space-y-2">
            {[
              ...
              <li key={i} className="grid grid-cols-[140px_1fr_60px] gap-3 items-center text-[12.5px]">
                <span className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: s.color }} />{s.k}</span>
                <div className="h-[7px] bg-[#EFE2C9] rounded-full overflow-hidden">
                  <div className="h-full" style={{ width: `${s.v}%`, background: s.color }} />
                </div>
...
function AdminApprovals() {
  const [rows, setRows] = useState(ADMIN_MARKET.approvalQueue);
    ...
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <h2 className="font-serif text-[22px] mt-1">
          {rows.filter((r) => r.status === "pending").length} pending · {rows.filter((r) => r.status === "needs-changes").length} need changes
        </h2>
        <p className="text-[12px] text-clay-700 mt-1">No auto-approval. Marketing copy must be honest. Affiliate links must resolve. Refund policy must be displayed.</p>
      </div>
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Listing</th><th>Seller</th><th>Type</th><th>Price</th><th>Submitted</th><th>Flags</th><th>Status</th><th className="right">Actions</th></tr>
          </thead>
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td className="num-mono text-clay-700">{r.submittedAt}</td>
                  ...
                  <td>
                    ...
                    {r.status === "approved" ? <TrustStamp kind="verified" /> : null}
                    {r.status === "needs-changes" ? <TrustStamp kind="preview" label="Needs changes" /> : null}
                    {r.status === "rejected" ? <TrustStamp kind="notcon" label="Rejected" /> : null}
                  </td>
                  <td className="right">
                    <div className="flex gap-1.5 justify-end flex-wrap">
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold whitespace-nowrap">Open detail</button>
                      ...
                      <button onClick={() => act(r.id, "needs-changes")} className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-clay-700 font-semibold whitespace-nowrap">Request changes</button>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-clay-700 mt-1">Bias toward approval. A free refund preserves trust; a contested one destroys it.</p>
      </div>
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Order</th><th>Product</th><th>Buyer</th><th>Amount</th><th>Reason</th><th>Window</th><th>State</th><th className="right">Actions</th></tr>
          </thead>
          <tbody>
            {ADMIN_MARKET.refundRequests.map((r) => (
              <tr key={r.id}>
                <td className="num-mono text-[10.5px] text-clay-700">{r.orderId}</td>
                ...
                <td>
                  ...
                  {r.state === "escalated" ? <TrustStamp kind="preview" label="Escalated" /> : null}
                  {r.state === "approved" ? <TrustStamp kind="verified" label="Approved" /> : null}
                  {r.state === "denied" ? <TrustStamp kind="notcon" label="Denied" /> : null}
                </td>
                <td className="right">
                  <div className="flex gap-1.5 justify-end flex-wrap">
                    ...
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-clay-700 font-semibold whitespace-nowrap">Partial · 50%</button>
...
function AdminPayouts() {
  const queued = ADMIN_MARKET.payouts.filter((p) => p.state === "queued");
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        <Eyebrow>Payouts · this cycle</Eyebrow>
        <h2 className="font-serif text-[22px] mt-1">
          May 30 · ₹{queued.reduce((a, b) => a + b.amount, 0).toLocaleString()} queued to {queued.length} sellers.
        </h2>
        <p className="text-[12px] text-clay-700 mt-1">Razorpay batch transfer at 14:00 IST. Holds release once flagged orders resolve.</p>
      </div>
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Seller</th><th>Amount</th><th>Cycle</th><th>State</th><th>Note</th><th className="right">Actions</th></tr>
          </thead>
          <tbody>
            {ADMIN_MARKET.payouts.map((p, i) => (
              <tr key={i}>
                <td><strong>{p.name}</strong><div className="num-mono text-[10.5px] text-clay-700">{p.sellerId}</div></td>
                <td className="num-mono">₹{p.amount.toLocaleString()}</td>
                <td className="num-mono">{p.cycle}</td>
                <td>
                  {p.state === "queued" ? <Pill tone="sage">queued · {p.scheduled}</Pill> : null}
                  {p.state === "hold" ? <Pill tone="amber">hold</Pill> : null}
                  {p.state === "affiliate" ? <Pill tone="dusk">affiliate · {p.scheduled}</Pill> : null}
                </td>
                <td className="text-clay-700">{p.reason || (p.state === "queued" ? "Refund window closed" : "")}</td>
                <td className="right">
                  <div className="flex gap-1.5 justify-end">
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold whitespace-nowrap">Open breakdown</button>
                    {p.state === "hold" ? <button className="text-[11px] px-2.5 py-1 rounded-full bg-[#33482F] text-[#F0F5EF] font-semibold whitespace-nowrap">Release hold</button> : null}
                  </div>
                </td>
              </tr>
            ))}
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-clay-700 mt-1">Misleading copy, price drift on affiliate links, copyright on PDFs.</p>
      </div>
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Severity</th><th>Listing</th><th>Seller</th><th>Reason</th><th>Flagged at</th><th className="right">Actions</th></tr>
          </thead>
          <tbody>
            {ADMIN_MARKET.flagged.map((f) => (
              <tr key={f.id}>
                <td>
                  {f.severity === "high" ? <Pill tone="rose">high</Pill> : null}
                  {f.severity === "medium" ? <Pill tone="amber">medium</Pill> : null}
                  {f.severity === "low" ? <Pill tone="outline">low</Pill> : null}
                </td>
                <td><strong>{f.title}</strong></td>
                <td className="num-mono text-clay-700">{f.seller}</td>
                <td className="text-[#3a2e22]">{f.reason}</td>
                <td className="num-mono text-clay-700">{f.at}</td>
                <td className="right">
                  <div className="flex gap-1.5 justify-end flex-wrap">
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold whitespace-nowrap">Open listing</button>
                    ...
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-clay-700 font-semibold whitespace-nowrap">Warn seller</button>
  ...
  return (
    <Card padded={false}>
      ...
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Seller</th><th>Trust</th><th>Listings</th><th>Rating</th><th>Platform cut</th><th className="right">Actions</th></tr>
          </thead>
          <tbody>
            {Object.values(SELLERS).map((s) => (
              <tr key={s.id}>
                <td>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="num-mono text-[10.5px] text-clay-700">{s.badge}</div>
                    </div>
                  </div>
                </td>
                <td>
                  <SellerTrustBadge
                    trust={s.kind === "first-party" ? "first-party" : s.kind === "topper" ? "verified-topper" : s.kind === "officer" ? "verified-officer" : s.kind}
                  />
                </td>
                ...
                <td className="right">
                  <div className="flex gap-1.5 justify-end flex-wrap">
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold whitespace-nowrap">Open profile</button>
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-clay-700 font-semibold whitespace-nowrap">Adjust cut</button>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {[
          ...
          <div key={i} className="rounded-xl border border-[#E7DECB] bg-white/70 p-4">
            ...
            <div className="num-mono text-[10.5px] text-clay-700 mt-1">partner since {a.since}</div>
            <div className="grid grid-cols-3 gap-3 mt-4">
              <Mini2 k="Listings" v={a.products} />
              <Mini2 k="Cut" v={`${a.cut}%`} />
              <Mini2 k="GMV · May" v={`₹${(a.gmv / 1000).toFixed(0)}k`} />
            </div>
            <div className="rule mt-3 pt-3 flex gap-2 flex-wrap">
              <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Open agreement</button>
              <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-clay-700 font-semibold">Adjust cut</button>
...
export default function PrototypeAdminMarket() {
  const [tab, setTab] = useState("overview");
  return (
    <PrototypePage label="Admin · Marketplace">
      ...
      <div className="px-10">
        ...
        <div className="mt-6 space-y-6">
          <AdminMarketKPIs />
          {tab === "overview" ? <AdminMarketOverview /> : null}
          {tab === "approvals" ? <AdminApprovals /> : null}
          {tab === "refunds" ? <AdminRefunds /> : null}
          {tab === "payouts" ? <AdminPayouts /> : null}
          {tab === "flagged" ? <AdminFlagged /> : null}
          {tab === "sellers" ? <AdminSellers /> : null}
          {tab === "affiliates" ? <AdminAffiliates /> : null}
        </div>
      </div>
      <FooterStrip />
    </PrototypePage>
