...
import { PRODUCTS, SELLER_DASH } from "../data/market";
import {
  Card, Eyebrow, FooterStrip, KPI, PageHeader, Pill, PrototypePage,
  SectionHeader, StatusDot, Tabs, TrustStamp,
} from "../ui";
...
function SellerOverview() {
  return (
    <>
      <Card>
        <SectionHeader eyebrow="Monthly revenue · last 6" title="Trend." sub="Bars are revenue after 20% platform fee." right={<StatusDot state="live" label="" />} />
        <svg viewBox="0 0 720 180" className="w-full h-[180px]">
          {[0, 15000, 30000, 45000].map((y, i) => (
            <g key={i}>
              ...
              <text x="52" y={150 - (y / 52000) * 120} textAnchor="end" dominantBaseline="central" fontFamily="JetBrains Mono" fontSize="9.5" fill="#6C5038">
                {y === 0 ? "0" : `₹${y / 1000}k`}
              </text>
            </g>
          ))}
        ...
        <div className="text-[10.5px] text-clay-700 mt-1">* partial month · pending payout</div>
      </Card>
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <SectionHeader eyebrow="Top listings · last 30 days" title="Where your revenue actually comes from." />
          <ul className="space-y-2.5">
            {SELLER_DASH.listings
              .filter((l) => l.sold30d > 0)
              ...
              .map((l) => {
                ...
                return (
                  <li key={l.id} className="grid grid-cols-[1fr_60px_80px] gap-3 items-center text-[12.5px]">
                    <span className="truncate">{p?.title || l.title}</span>
                    <span className="num-mono text-clay-700 text-right">{l.sold30d} sold</span>
        ...
        <Card className="!bg-[#2E2218] !border-[#2E2218]">
          <Eyebrow dark>Payout · next cycle</Eyebrow>
          ...
          <div className="rule mt-4 pt-3 text-[11.5px] text-[#D6BC93] space-y-1.5" style={{ borderColor: "#4E3A29" }}>
      ...
      <Card>
        <SectionHeader eyebrow="Recent orders · last 24h" title="Live buyer activity." right={<StatusDot state="live" label="" />} />
        <div className="overflow-x-auto">
          <table className="tbl">
            <thead>
              <tr><th>When</th><th>Buyer</th><th>Product</th><th>Amount</th><th>You earn</th><th className="right">Ref</th></tr>
            </thead>
            <tbody>
              {SELLER_DASH.recentOrders.map((o, i) => (
                <tr key={i}>
                  <td className="num-mono text-clay-700">{o.at}</td>
                  <td>{o.buyer}</td>
                  <td>{o.product}</td>
                  ...
                  <td className="right num-mono text-[10.5px] text-clay-700">{o.ref}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3 flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[12px] text-clay-700 mt-1">New listings require admin approval before going live. Drafts are private to you.</p>
        </div>
      ...
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Listing</th><th>Type</th><th>Price</th><th>Sold · 30d</th><th>Revenue · 30d</th><th>Rating</th><th>Status</th><th className="right">Actions</th></tr>
          </thead>
          <tbody>
            {SELLER_DASH.listings.map((l) => {
              ...
              return (
                <tr key={l.id}>
                  <td>
                    <strong>{title}</strong>
                    {l.blockedBy ? <div className="text-[10.5px] text-[#7A3925] mt-0.5">⛔ {l.blockedBy}</div> : null}
                  </td>
                  ...
                  <td>
                    {l.status === "live" ? <TrustStamp kind="live" label="Live" /> : null}
                    {l.status === "draft" ? <TrustStamp kind="preview" label="Draft" /> : null}
                    {l.status === "paused" ? <TrustStamp kind="notcon" label="Paused" /> : null}
                    {l.status === "review" ? <TrustStamp kind="needs" label="In review" /> : null}
                  </td>
                  <td className="right">
                    <div className="flex gap-1.5 justify-end">
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Edit</button>
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Analytics</button>
                      {l.status === "live" ? <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-clay-700 font-semibold">Pause</button> : null}
                      {l.status === "draft" ? <button className="text-[11px] px-2.5 py-1 rounded-full bg-[#33482F] text-[#F0F5EF] font-semibold">Submit for review</button> : null}
                    </div>
                  </td>
                </tr>
              );
  ...
  return (
    <Card padded={false}>
      ...
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>When</th><th>Buyer</th><th>Product</th><th>Amount</th><th>Your earn</th><th>Status</th><th className="right">Ref</th></tr>
          </thead>
          <tbody>
            {orders.map((o, i) => (
              <tr key={i}>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-clay-700 mt-1">Platform fee is 20% on first-party hosted products. GST is collected and remitted on your behalf.</p>
      </div>
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Date</th><th>Amount</th><th>Reference</th><th>Status</th><th>Notes</th><th className="right">Receipt</th></tr>
          </thead>
          <tbody>
            <tr>
              ...
              <td className="num-mono text-clay-700">PAY-2026-05-K42</td>
              <td><Pill tone="amber">queued</Pill></td>
              <td className="text-clay-700">2 orders still in refund window</td>
              <td className="right text-clay-700">—</td>
            </tr>
            {SELLER_DASH.payouts.map((p, i) => (
              <tr key={i}>
                ...
                <td className="text-clay-700">{p.net}</td>
                <td className="right"><button className="text-[11px] text-clay-700 hover:text-clay-900 underline">Download</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {SELLER_DASH.reviewsRecent.map((r, i) => (
          <li key={i} className="rounded-xl border border-[#E7DECB] bg-white/70 p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="num-mono text-[10.5px] text-clay-700">{r.buyer} · {r.at}</div>
            </div>
            ...
            <div className="rule mt-3 pt-3 flex gap-2 flex-wrap">
              ...
              <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Thank buyer</button>
...
export default function PrototypeSeller() {
  const [tab, setTab] = useState("overview");
  return (
    <PrototypePage label="Seller dashboard">
      <div className="px-10 pt-9">
        <PageHeader
          eyebrow="Seller · you are Kavya Iyer"
          ...
          right={
            <div className="flex gap-2">
              <button className="text-[12px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-clay-700 font-semibold">Public profile</button>
              <button className="text-[12px] px-3 py-1.5 rounded-full bg-[#2E2218] text-[#F3EADB] font-semibold">+ New listing</button>
            </div>
          }
        />
      ...
      <div className="px-10 space-y-6">
        ...
        />
        {tab === "overview" ? <SellerOverview /> : null}
        {tab === "listings" ? <SellerListings /> : null}
        {tab === "orders" ? <SellerOrders /> : null}
        {tab === "payouts" ? <SellerPayouts /> : null}
        {tab === "reviews" ? <SellerReviews /> : null}
      </div>
      <FooterStrip />
    </PrototypePage>
  );
