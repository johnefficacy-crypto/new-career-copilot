...
import { FUNNEL_ANALYTICS } from "../data/funnel";
import {
  Card, Eyebrow, FooterStrip, KPI, PageHeader, Pill, PrototypePage,
  SectionHeader, StatusDot, TrustStamp,
} from "../ui";
      ...
      <div className="px-7 pt-6 pb-3 flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <h2 className="font-serif text-[22px] mt-1">
            {A.conversion[0].count.toLocaleString()} sessions → {A.conversion[A.conversion.length - 1].count.toLocaleString()} S7 completes.
          </h2>
          <p className="text-[12px] text-clay-700 mt-1">Each step shows count, % of previous, and % of total. Watch the drop between S5 PEEK and S6 success.</p>
        </div>
        ...
        {A.conversion.map((s, i) => {
          ...
          return (
            <div key={i} className="grid grid-cols-[160px_1fr_120px] gap-4 items-center">
              <div>
                <div className="num-mono text-[10.5px] text-clay-700 uppercase tracking-[0.14em]">{s.stage}</div>
                <div className="text-[11.5px] text-clay-900">{s.label}</div>
              </div>
              <div className="relative">
                <div className="h-[26px] bg-[#EFE2C9] rounded-md overflow-hidden">
                  <div className="h-full transition-all" style={{ width: `${widthPct}%`, background: i === A.conversion.length - 1 ? "#33482F" : "#54794E" }} />
                </div>
              ...
              <div className="num-mono text-[11px] text-right">
                <div className="text-clay-900">{totalPct}% total</div>
                {i > 0 ? <div className={dropPct > 20 ? "text-[#7A3925]" : "text-clay-700"}>−{dropPct}% step</div> : null}
              </div>
            </div>
          );
        })}
...
function DropOffByQuestion({ A }) {
  return (
    <Card>
      <SectionHeader eyebrow="Drop-off by question" title="Where users leave." sub="Parser-rejection rates and copy issues show up here first." />
      <ul className="space-y-2.5">
        {A.drop_off_by_question.map((d, i) => (
          <li key={i} className="grid grid-cols-[120px_1fr_50px] gap-3 items-center text-[12.5px]">
            <span className="num-mono text-clay-700">{d.q}</span>
            <div className="relative">
              <div className="h-[8px] bg-[#EFE2C9] rounded-full overflow-hidden">
                <div className="h-full" style={{ width: `${d.drop * 100 * 4}%`, background: d.drop > 0.12 ? "#7A3925" : d.drop > 0.07 ? "#A68057" : "#94B28A" }} />
              </div>
              {d.note ? <div className="text-[10.5px] text-[#7A3925] mt-1">⚠ {d.note}</div> : null}
            </div>
...
function IntentSplit({ A }) {
  ...
  return (
    <Card>
      <SectionHeader eyebrow="Intent split · today" title="What users said they came for." sub="CTA-funneled traffic skips S2 — their intent is set by the URL." />
      <div className="flex items-center gap-5 flex-wrap">
        <svg width="160" height="160" viewBox="0 0 160 160">
          ...
          <text x="80" y="80" textAnchor="middle" dominantBaseline="central" fontFamily="Fraunces" fontSize="22" fontWeight="600" fill="#2E2218">
            {A.today.sessions_started > 1000 ? `${(A.today.sessions_started / 1000).toFixed(1)}k` : A.today.sessions_started}
          </text>
        </svg>
        <ul className="flex-1 space-y-2 text-[12.5px] min-w-[180px]">
          {A.intents.map((i, idx) => (
            <li key={idx} className="grid grid-cols-[18px_1fr_50px] gap-2 items-center">
              ...
              <span className="num-mono text-right text-clay-700">{i.pct}%</span>
            </li>
          ))}
        </ul>
      </div>
...
function TimeHistogram({ A }) {
  return (
    <Card>
      <SectionHeader eyebrow="Time-to-S7 distribution" title={`Median ${A.today.median_questions_answered} questions · ${A.today.avg_time_to_s7} average.`} />
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-clay-700 mt-1">CTA-funneled traffic from blogs is the highest-converting channel.</p>
      </div>
          ...
          <tbody>
            {A.source.map((s, i) => (
              <tr key={i}>
                ...
                <td><span className={`num-mono ${s.conv >= 0.3 ? "text-[#33482F] font-semibold" : "text-clay-700"}`}>{(s.conv * 100).toFixed(0)}%</span></td>
                <td className="num-mono text-clay-700">{s.intent}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-clay-700 mt-1">Every stitch is logged with anon id, user id, and three timestamps.</p>
      </div>
      <div className="px-2 overflow-x-auto">
        <table className="tbl">
          <thead>
            <tr><th>Session</th><th>Anonymous id</th><th>Stitched user</th><th>t · anon</th><th>t · login</th><th>t · done</th><th>State</th><th>Source</th></tr>
          </thead>
          <tbody>
            {A.stitch_audit.map((s, i) => (
              <tr key={i}>
                <td className="num-mono">{s.id}</td>
                <td className="num-mono text-clay-700">{s.anon}</td>
                <td className="num-mono">{s.stitched_user || <span className="text-[#7A3925]">— not stitched</span>}</td>
                <td className="num-mono text-clay-700">{s.t_anon}</td>
                <td className="num-mono text-clay-700">{s.t_login || "—"}</td>
                <td className="num-mono text-clay-700">{s.t_done || "—"}</td>
                <td>
                  {s.state === "S7" ? <TrustStamp kind="verified" label="S7 done" /> : null}
                  {s.paused ? <TrustStamp kind="preview" label="S8 paused" /> : null}
                  {!s.paused && s.state !== "S7" ? <TrustStamp kind="needs" label={s.state} /> : null}
                </td>
                <td className="num-mono text-clay-700">{s.source || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function PrototypeAdminFunnel() {
  const A = FUNNEL_ANALYTICS;
  return (
    <PrototypePage label="Admin · Funnel analytics">
      <div className="px-10 pt-9">
        <PageHeader
          eyebrow="Admin · Funnel"
