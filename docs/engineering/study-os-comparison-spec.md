...
- **Backend accountability router** (`app/backend/app/api/placeholders.py`, `router_acc`, prefix `/accountability`) — still returns hardcoded mock data. The canonical-router file (`app/backend/app/api/canonical.py`) explicitly flags this: *"accountability (no canonical partner/group tables exist yet)"*. PRs 6 / 8 / 10 below must replace these placeholders with Supabase-backed handlers, not add a second router.
