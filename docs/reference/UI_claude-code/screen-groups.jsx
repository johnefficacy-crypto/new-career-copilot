/* /app/groups — Study groups list + detail with sessions */
const { useState: useStateG } = React;

function ScreenGroups() {
  const [tab, setTab] = useStateG("mine");
  const [activeId, setActiveId] = useStateG("g1");
  ...
  return (
    <div data-screen-label="Study Groups">
      <PageHeader eyebrow="Study Groups"
        ...
        right={
          <div className="flex gap-2">
            <button className="text-[12px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Find a group →</button>
      ...
      <div className="px-10 space-y-6">
        <div className="flex items-end justify-between gap-3 flex-wrap">
          ...
          <div className="num-mono text-[10.5px] text-[#6C5038]">{filtered.length} groups · {STUDY_ROOM_SESSIONS.length} study rooms this week</div>
        </div>
        ...
        <div className="grid grid-cols-[360px_1fr] gap-6">
          <div className="space-y-3">
            ...
            {filtered.length === 0 && <EmptyState icon="◇" title="No groups in this view." body="Try a different tab or create your own." />}
          </div>
...
function GroupListCard({ g, active, onPick }) {
  ...
  return (
    <button onClick={onPick}
      className={`w-full text-left rounded-xl border p-4 transition ${active ? 'bg-[#FBF6EF] border-[#2E2218]' : 'bg-white/70 border-[#E7DECB] hover:border-[#A68057]'}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="font-serif text-[16px] leading-tight">{g.name}</div>
          <div className="num-mono text-[10.5px] text-[#6C5038] mt-1">{g.exam} · {g.schedule}</div>
        </div>
        <Pill tone={g.status === 'active' ? 'sage' : g.status === 'paused' ? 'amber' : 'outline'}>{g.status}</Pill>
      </div>

      <div className="mt-3 flex items-center gap-4 text-[10.5px] text-[#6C5038]">
        <span className="num-mono">{g.members}/{g.capacity} members</span>
        <span>·</span>
        <span>{g.visibility}</span>
        {g.isMine && <Pill tone="ink" className="!text-[9px]">Mine</Pill>}
      </div>

      <div className="mt-3">
        <div className="flex items-center justify-between text-[10px] text-[#6C5038]">
        ...
        <div className="mt-1 h-[5px] bg-[#EFE2C9] rounded-full overflow-hidden">
          <div className="h-full bg-[#54794E]" style={{width: Math.min(100, Math.round(pctH*100)) + "%"}}></div>
        </div>
      </div>
...
function GroupDetail({ group }) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-[12.5px] text-[#6C5038] mt-1.5">{group.schedule} · founded by {COMMUNITY_USERS[group.founder]?.name}</div>
          </div>
          <div className="flex gap-2 shrink-0">
            <button className="text-[11.5px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Group settings</button>
            {group.isMine ? <Pill tone="ink">You're in</Pill> : <button className="text-[11.5px] px-3 py-1.5 rounded-full bg-[#2E2218] text-[#F3EADB] font-semibold">Request to join</button>}
          </div>
        </div>
      ...
      {group.nextSession && <NextSessionCard s={group.nextSession} />}

      <div className="grid grid-cols-2 gap-6">
        <DailyCheckinCard group={group} />
...
function GroupKPI({ k, v, sub, pct }) {
  return (
    <div className="rounded-xl border border-[#E7DECB] bg-[#FBF8F2] p-3.5">
      ...
      <div className="text-[11px] text-[#6C5038] mt-1.5">{sub}</div>
      {pct != null && (
        <div className="mt-2 h-[5px] bg-[#EFE2C9] rounded-full overflow-hidden">
          <div className="h-full bg-[#54794E]" style={{width: Math.min(100, Math.round(pct*100)) + "%"}}></div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <Card className="!bg-[#2E2218] !border-[#2E2218]">
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 border-l border-[#4E3A29] pl-5">
          ...
          <div className="num-mono text-[10.5px] text-[#A68057] mt-2.5">platform set by group founder · platform link visible after RSVP</div>
        </div>
      </div>
    </Card>
  );
...
function DailyCheckinCard({ group }) {
  return (
    <Card>
      ...
      <div className="rounded-xl border border-[#E7DECB] bg-[#FBF8F2] p-3.5">
        <div className="flex items-center gap-2 num-mono text-[10.5px] text-[#6C5038] uppercase tracking-[0.16em]">
          <span>Today · May 14</span><span>·</span><span className="text-[#33482F]">4 of 6 checked in</span>
        </div>
        <textarea rows="2" placeholder="Federalism revision · Mock 13 errors reviewed · 5.5h focused"
          className="mt-2 w-full bg-transparent outline-none text-[13px] placeholder:text-[#A68057] resize-none"></textarea>
        <div className="flex items-center justify-between mt-2">
          <div className="flex gap-1.5">
            <button className="text-[10.5px] px-2 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Hours: 5.5</button>
            <button className="text-[10.5px] px-2 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">+ Topic</button>
          </div>
            ...
            <li key={i} className="grid grid-cols-[28px_1fr_60px] gap-3 items-start">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {c.skipped && <Pill tone="amber" className="!text-[9px]">Skipped</Pill>}
                </div>
                <div className="text-[12px] text-[#3a2e22] mt-0.5">{c.body}</div>
              </div>
              <span className="num-mono text-[10.5px] text-[#6C5038] text-right">{c.t}</span>
            </li>
          );
        })}
      </ul>
  ...
  return (
    <Card>
      ...
      <ul className="space-y-2.5">
        ...
        ].slice(0, group.members).map((m,i) => {
          const u = COMMUNITY_USERS[m.u];
          return (
            <li key={i} className="grid grid-cols-[32px_1fr_60px_50px] gap-3 items-center">
              <Avatar user={u} size={28} />
              <div className="min-w-0">
                <div className="flex items-center gap-1.5"><span className="text-[12.5px] font-medium truncate">{u.name}</span>{m.founder && <Pill tone="ink" className="!text-[9px]">founder</Pill>}<UserBadge user={u} compact /></div>
                <div className="num-mono text-[10.5px] text-[#6C5038]">joined {m.join}</div>
              </div>
              <MiniBar pct={m.hrs/45} width={50} />
              <span className="num-mono text-[11px] text-[#6C5038] text-right">{m.hrs}h</span>
            </li>
          );
        })}
      </ul>
...
function SharedResourcesCard() {
  return (
    <Card>
      <SectionHeader eyebrow="Shared resource library" title="Group-visible files & links."
      ...
      <ul className="grid grid-cols-3 gap-3">
        {[
          ...
          <li key={i} className="rounded-xl border border-[#E7DECB] bg-[#FBF8F2] p-3.5">
            <div className="flex items-center justify-between"><SourceTrustStamp trust={r.trust} /><span className="num-mono text-[10.5px] text-[#6C5038]">↑ {r.up}</span></div>
            <div className="font-serif text-[14px] mt-2 leading-snug">{r.t}</div>
            <div className="num-mono text-[10.5px] text-[#6C5038] mt-1">shared by {COMMUNITY_USERS[r.by]?.name}</div>
          </li>
        ))}
      </ul>
    </Card>
...
function PostSessionLogCard() {
  return (
    <Card>
      <SectionHeader eyebrow="Post-session log"
      ...
      <table className="tbl mt-1">
        <thead><tr><th>Session</th><th>Hours logged</th><th>Topics covered</th><th>Attended</th><th>Notes</th></tr></thead>
        <tbody>
          <tr><td><strong>Federalism revision</strong><div className="num-mono text-[10.5px] text-[#6C5038]">May 13 · 06:00</div></td><td className="num-mono">2.0h</td><td>Centre–State · Emergency</td><td className="num-mono">5/6</td><td>Strong session</td></tr>
          <tr><td><strong>Mock 13 walkthrough</strong><div className="num-mono text-[10.5px] text-[#6C5038]">May 11 · 19:00</div></td><td className="num-mono">2.5h</td><td>Polity · History · Eco</td><td className="num-mono">6/6</td><td>Identified shared weak areas</td></tr>
          <tr><td><strong>Polity Ch. 4 revision</strong><div className="num-mono text-[10.5px] text-[#6C5038]">May 09 · 06:00</div></td><td className="num-mono">1.8h</td><td>Polity Ch.4</td><td className="num-mono">4/6</td><td>Two members absent</td></tr>
        </tbody>
      </table>
    </Card>
  );
    ...
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3 flex items-end justify-between">
        <div>
          ...
          <p className="text-[12px] text-[#6C5038] mt-1">Reminders 15 min before. Post-session hours feed your study analytics.</p>
        </div>
        <button className="text-[11.5px] px-3 py-1.5 rounded-full bg-[#2E2218] text-[#F3EADB] font-semibold">+ Schedule a session</button>
      </div>
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>Title</th><th>When</th><th>Duration</th><th>Group</th><th>Platform</th><th>Confirmed</th><th className="right"></th></tr></thead>
          <tbody>
            {STUDY_ROOM_SESSIONS.map(s => {
              const g = STUDY_GROUPS.find(x=>x.id===s.groupId);
              return (
                <tr key={s.id}>
                  <td><strong>{s.title}</strong><div className="num-mono text-[10.5px] text-[#6C5038]">{s.agenda}</div></td>
                  <td className="num-mono">{s.at}</td>
                  <td className="num-mono">{s.duration}</td>
                  <td className="text-[#6C5038]">{g?.name}</td>
                  <td><Pill tone="outline">{s.platform}</Pill></td>
                  <td className="num-mono">{s.confirmed}/{s.maxParticipants}</td>
                  <td className="right">
                    <div className="flex gap-1.5 justify-end">
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">RSVP</button>
    ...
    <Card className="!bg-[#F0F5EF] !border-[#B9CFAF]">
      <div className="flex items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-[12.5px] text-[#33482F] mt-1">One person, daily ✅, weekly review. Less coordination, more pressure.</p>
        </div>
        <a href="#partners" className="text-[12.5px] px-3.5 py-2 rounded-full bg-[#33482F] text-[#F0F5EF] font-semibold">Open partners →</a>
      </div>
    </Card>
  );
}
