/* /admin/eligibility — verification console for eligibility criteria */
...
const AE_DATA = {
  overview: {
    openRecruitments: 14, criteriaVerified: 86, unverified: 9,
    affectedAspirants: 12480, lowConfidence: 4, recompute: "Today · 04:48",
  },
  recruits: [
  ...
  queue: [
    { id:"c1", recruit:"BARC OCES 2026", facet:"GATE pathway",
      scraped:"GATE CS/EE 2023/24 acceptable as primary shortlist (Engg cadres)",
      conf:0.74, source:"barc.gov.in/notif-2026.pdf §4.2",
      affected:1820, status:"pending" },
    { id:"c2", recruit:"UPPSC 2026", facet:"Education discipline",
      scraped:"Any Bachelor's degree from a recognized university acceptable for PCS",
      conf:0.92, source:"uppsc.up.nic.in/notification §III(b)",
      affected:9410, status:"pending" },
    { id:"c3", recruit:"Indian Forest 2026", facet:"Engineering discipline mapping",
      scraped:"CSE not explicitly listed; B.E./B.Tech with Computer Science under \"Engineering\"",
      conf:0.58, source:"upsc.gov.in/ifos-2026.pdf §VI",
      affected:540, status:"pending" },
    { id:"c4", recruit:"MPSC 2026", facet:"Age relaxation matrix",
      scraped:"Backwards classes +3y; Maharashtra govt servants +5y (max 43)",
      conf:0.88, source:"mpsc.gov.in/notification §2.1",
      affected:2210, status:"pending" },
    { id:"c5", recruit:"MPSC 2026", facet:"Domicile",
      scraped:"Maharashtra domicile mandatory for posts marked (R); non-domicile for general",
      conf:0.91, source:"mpsc.gov.in/notification §3",
      affected:2210, status:"pending" },
    { id:"c6", recruit:"RRB NTPC 2026", facet:"Attempt limits",
      scraped:"No defined upper attempt limit; age-only constraint",
      conf:0.69, source:"rrbcdg.gov.in/notif §5.3",
      affected:3650, status:"pending" },
    { id:"c7", recruit:"RRB NTPC 2026", facet:"Education requirement",
      scraped:"Graduate / 12th-pass split by post group; ambiguity on group 'C'",
      conf:0.55, source:"rrbcdg.gov.in/notif §2",
      affected:3650, status:"pending" },
    { id:"c8", recruit:"UPPSC 2026", facet:"Domicile benefit",
      scraped:"UP-domicile candidates receive +5 marks in interview; reservation in state quota",
      conf:0.82, source:"uppsc.up.nic.in §IV",
      affected:9410, status:"pending" },
    { id:"c9", recruit:"BARC OCES 2026", facet:"Attempts",
      scraped:"No restriction; age cap controls eligibility",
      conf:0.85, source:"barc.gov.in/notif-2026.pdf §4.5",
      affected:1820, status:"pending" },
  ],
  sources: [
...
function ScreenAdminEligibility() {
  const [tab, setTab] = useStateAE("overview");
  const [drawer, setDrawer] = useStateAE(null);
  return (
    <div data-screen-label="Admin · Eligibility verification">
      <PageHeader eyebrow="Admin · Eligibility verification"
      ...
      <div className="px-10">
        ...
        <div className="mt-6 space-y-6">
          {tab === "overview" && <AEOverview />}
          {tab === "recruits" && <AERecruitments onOpenSource={(r)=>setDrawer({kind:'source', row:r})} />}
          {tab === "queue"    && <AECriteriaQueue onOpenEvidence={(r)=>setDrawer({kind:'evidence', row:r})} />}
          {tab === "sources"  && <AESources />}
          {tab === "impact"   && <AEMatchImpact />}
        </div>
      </div>

      {drawer && drawer.kind === 'evidence' && (
        <EvidenceDrawer open={true} onClose={()=>setDrawer(null)} title={`Evidence · ${drawer.row.facet}`} items={[
      ...
      {drawer && drawer.kind === 'source' && (
        <Drawer open={true} onClose={()=>setDrawer(null)} title={drawer.row.name}>
          <div className="space-y-3">
            <div className="rounded-xl border border-[#E7DECB] bg-white/60 p-4">
              ...
              <div className="num-mono text-[10.5px] text-[#6C5038] mt-1">{drawer.row.deadline} · {drawer.row.family}</div>
            </div>
            <div className="rounded-xl border border-[#E7DECB] bg-white/60 p-4">
              <Eyebrow>Verified facets</Eyebrow>
              <div className="text-[15px] font-serif mt-1">{drawer.row.verified} / {drawer.row.total}</div>
              {drawer.row.note && <div className="text-[11.5px] text-[#7A3925] mt-2">⚠ {drawer.row.note}</div>}
            </div>
            <div className="flex gap-2">
              <button className="text-[12px] px-3 py-1.5 rounded-full bg-[#2E2218] text-[#F3EADB] font-semibold">Open all criteria</button>
              <button className="text-[12px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Trigger re-scrape</button>
            </div>
          </div>
        </Drawer>
      )}
    ...
    <Card padded={false}>
      ...
      <div className="px-2">
        <table className="tbl">
          <thead>
            <tr>
              <th>Recruitment</th><th>Family</th><th>Deadline</th><th>Criteria verified</th>
              <th>Source</th><th>Last reviewed</th><th>Status</th><th className="right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {AE_DATA.recruits.map((r,i) => (
              <tr key={i}>
                <td>
                  <div className="font-medium text-[#2E2218]">{r.name}</div>
                  {r.note && <div className="text-[11px] text-[#7A3925] mt-1">⚠ {r.note}</div>}
                </td>
                ...
                <td>
                  <div className="flex items-center gap-2">
                    <div className="h-[5px] bg-[#EFE2C9] rounded-full overflow-hidden" style={{width:80}}>
                      <div className="h-full" style={{width:`${(r.verified/r.total)*100}%`, background: r.verified === r.total ? '#54794E' : '#A68057'}}></div>
                    </div>
                    <span className="num-mono text-[11px]">{r.verified}/{r.total}</span>
                  </div>
                </td>
                <td><span className="num-mono text-[11px]">{r.source}</span></td>
                <td className="num-mono text-[#6C5038]">{r.lastReviewed}</td>
                <td>
                  {r.status === 'verified' && <TrustStamp kind="verified" />}
                  {r.status === 'partial'  && <TrustStamp kind="preview" label="Partial" />}
                  {r.status === 'pending'  && <TrustStamp kind="needs" label="Pending" />}
                  {r.status === 'closed'   && <TrustStamp kind="notcon" label="Closed" />}
                </td>
                <td className="right">
                  <button onClick={()=>onOpenSource(r)} className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold whitespace-nowrap">Open →</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function AECriteriaQueue({ onOpenEvidence }) {
  const [rows, setRows] = useStateAE(AE_DATA.queue);
    ...
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3 flex items-end justify-between">
        <div>
          ...
          <p className="text-[12px] text-[#6C5038] mt-1">Each criterion controls verdicts for thousands of aspirants. Verifying a criterion re-runs the eligibility engine for affected users.</p>
        </div>
        <div className="flex gap-2">
            ...
            {rows.map(r => (
              <tr key={r.id}>
                ...
                <td>
                  ...
                  <div className="num-mono text-[10.5px] text-[#6C5038] mt-1">{r.source}</div>
                </td>
                ...
                <td className="right">
                  <div className="flex gap-1.5 justify-end flex-wrap">
                    <button onClick={()=>onOpenEvidence(r)} className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold whitespace-nowrap">Evidence</button>
                    ...
                    <button onClick={()=>act(r.id,'correction')} className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-[#6C5038] font-semibold whitespace-nowrap">Re-scrape</button>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-[#6C5038] mt-1">Official sources can flow to verdicts after review. Aggregator sources are discovery only — never auto-promote.</p>
      </div>
      <div className="px-7 pb-6 grid grid-cols-2 gap-5">
        <div>
...
function SourceRow({ s }) {
  return (
    <li className="rounded-xl border border-[#E7DECB] bg-[#FBF8F2] px-3.5 py-3 flex items-center gap-3">
      <span className={`sdot ${s.health === 'green' ? 'sdot-live' : s.health === 'amber' ? 'sdot-partial' : 'sdot-not'}`}></span>
      <div className="flex-1 min-w-0">
        <div className="font-mono text-[13px]">{s.name}</div>
        <div className="num-mono text-[10.5px] text-[#6C5038]">{s.freq} · last {s.last} · {s.items} items</div>
        {s.warn && <div className="text-[10.5px] text-[#7A3925] mt-0.5">⚠ {s.warn}</div>}
        {s.note && <div className="text-[10.5px] text-[#524864] mt-0.5">· {s.note}</div>}
      </div>
      <button className="text-[10.5px] px-2 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold whitespace-nowrap">Force scrape</button>
    </li>
  );
}
...
function AEMatchImpact() {
  return (
    <Card>
      <SectionHeader eyebrow="Match impact · simulate before commit"
        ...
        right={
          <div className="flex gap-2">
            ...
            <button className="text-[12px] px-3 py-1.5 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Discard</button>
          </div>
        } />
      <div className="grid grid-cols-[280px_1fr] gap-6">
        <div>
          ...
          <div className="rounded-xl border border-[#2E2218] bg-[#FBF6EF] p-3 mt-2">
            <div className="num-mono text-[10.5px] text-[#6C5038]">BARC OCES 2026</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl border border-[#E7DECB] bg-[#FBF8F2] p-4">
            ...
            <ul className="mt-2.5 space-y-1.5 text-[12.5px]">
              ...
              <li className="flex justify-between text-[#6C5038] text-[11px]"><span>(others · no GATE)</span><span className="num-mono">—</span></li>
            </ul>
          </div>
...
window.ScreenAdminEligibility = ScreenAdminEligibility;
