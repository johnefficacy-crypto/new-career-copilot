/* /admin/community — Moderation, mentor apps, badges, channel config */
...
function ScreenAdminCommunity() {
  const [tab, setTab] = useStateAC("reports");
  return (
    <div data-screen-label="Admin · Community">
      <PageHeader eyebrow="Admin · Community"
      ...
      <div className="px-10">
        ...
        <div className="mt-6 space-y-6">
          <ACMetrics />
          {tab === "reports" && <ACReports />}
          {tab === "mods" && <ACModActions />}
          {tab === "channels" && <ACChannels />}
          {tab === "mentors" && <ACMentorApps />}
          {tab === "sessions" && <ACMentorSessions />}
          {tab === "resources" && <ACResourceQueue />}
          {tab === "badges" && <ACBadges />}
        </div>
      </div>
...
function ACReports() {
  const [rows, setRows] = useStateAC(ADMIN_COMM.reports);
  function act(id, state) { setRows(rs => rs.map(r => r.id === id ? {...r, state} : r)); }
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        <Eyebrow>Pending reports</Eyebrow>
        <h2 className="font-serif text-[22px] mt-1">{rows.filter(r=>r.state==='open').length} open · {rows.filter(r=>r.state==='action-pending').length} awaiting action</h2>
      </div>
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>Severity</th><th>Target</th><th>Title</th><th>Reason</th><th>Reporters</th><th>At</th><th>State</th><th className="right">Actions</th></tr></thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.id}>
                <td>
                  {r.severity === 'high' && <Pill tone="rose">high</Pill>}
                  {r.severity === 'medium' && <Pill tone="amber">medium</Pill>}
                  {r.severity === 'low' && <Pill tone="outline">low</Pill>}
                </td>
                <td><Pill tone="outline">{r.target}</Pill></td>
                <td><strong>{r.targetTitle}</strong></td>
                <td className="text-[#6C5038]">{r.reason}</td>
                <td className="num-mono">{r.reportedBy}</td>
                <td className="num-mono text-[#6C5038]">{r.at}</td>
                <td>
                  {r.state === 'open' && <TrustStamp kind="needs" label="Open" />}
                  {r.state === 'action-pending' && <TrustStamp kind="preview" label="Action pending" />}
                  {r.state === 'resolved' && <TrustStamp kind="verified" label="Resolved" />}
                </td>
                <td className="right">
                  <div className="flex gap-1.5 justify-end">
                    ...
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#BE9C6B] text-[#6C5038] font-semibold whitespace-nowrap">Warn user</button>
...
function ACModActions() {
  return (
    <Card>
      <SectionHeader eyebrow="Moderation log" title="Last 24 hours." right={<StatusDot state="live" />} />
      <table className="tbl">
        <thead><tr><th>When</th><th>Action</th><th>By</th><th>Target</th><th>Reason</th><th className="right">Reversible?</th></tr></thead>
        <tbody>
...
function ACChannels() {
  return (
    <Card padded={false}>
      ...
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>Channel</th><th>Locked admin-write</th><th>Channel admins</th><th>Auto-sync · admin/exam-intelligence</th><th className="right">Actions</th></tr></thead>
          <tbody>
            {ADMIN_COMM.channelsConfig.map(c => (
              <tr key={c.id}>
                ...
                <td className="right">
                  <div className="flex gap-1.5 justify-end">
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Edit rules</button>
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Manage admins</button>
                  </div>
                </td>
              </tr>
            ))}
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-[#6C5038] mt-1">A scorecard or DOPT page is required. Mentors are not auto-approved.</p>
      </div>
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>Applicant</th><th>Submitted</th><th>Claim</th><th>Proof</th><th>Topics</th><th>Status</th><th className="right">Actions</th></tr></thead>
          <tbody>
            {ADMIN_COMM.mentorApplications.map(a => {
              ...
              return (
                <tr key={a.id}>
                  ...
                  <td className="num-mono text-[#6C5038]">{a.at}</td>
                  <td className="num-mono">{a.rank}</td>
                  <td className="text-[#6C5038]">{a.proof}</td>
                  <td className="text-[#3a2e22]">{a.topics}</td>
                  <td>
                    {a.status === 'pending' && <TrustStamp kind="needs" label="Pending review" />}
                    {a.status === 'verified' && <TrustStamp kind="verified" />}
                    {a.status === 'rejected' && <TrustStamp kind="notcon" label="Rejected" />}
                  </td>
                  <td className="right">
                    <div className="flex gap-1.5 justify-end">
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Open evidence</button>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-[#6C5038] mt-1">Cancellation triggers automatic refund to all booked aspirants.</p>
      </div>
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>Session</th><th>Mentor</th><th>When</th><th>Booked</th><th>Price</th><th>Platform</th><th>Status</th><th className="right">Actions</th></tr></thead>
          <tbody>
            {MENTOR_SESSIONS.map(s => {
              ...
              return (
                <tr key={s.id}>
                  ...
                  <td className="right">
                    <div className="flex gap-1.5 justify-end">
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Open</button>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-[#6C5038] mt-1">Pirated paid material is removed regardless of upvotes. Verify-by-Topper is set here.</p>
      </div>
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>Resource</th><th>Type</th><th>Trust</th><th>Contributor</th><th>Upvotes</th><th>Flagged</th><th className="right">Actions</th></tr></thead>
          <tbody>
            {RESOURCES.map(r => (
              <tr key={r.id}>
                <td><strong>{r.title}</strong><div className="num-mono text-[10.5px] text-[#6C5038]">{r.exam} · {r.subject}</div></td>
                ...
                <td className="right">
                  <div className="flex gap-1.5 justify-end">
                    ...
                    <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold whitespace-nowrap">Verify-by-Topper</button>
  ...
  return (
    <Card padded={false}>
      <div className="px-7 pt-6 pb-3">
        ...
        <p className="text-[12px] text-[#6C5038] mt-1">Every grant is evidence-backed and reversible. The badge changes everything about how a user is heard — handle carefully.</p>
      </div>
      <div className="px-2">
        <table className="tbl">
          <thead><tr><th>User</th><th>Requested badge</th><th>Evidence</th><th>Submitted</th><th className="right">Actions</th></tr></thead>
          <tbody>
            {ADMIN_COMM.badges.pending.map((b,i) => {
              ...
              return (
                <tr key={i}>
                  <td><div className="flex items-center gap-2"><Avatar user={u} size={26} /><span className="font-medium">{u.name}</span></div></td>
                  <td>
                    {b.kind === "topper"  && <VerifiedTopperBadge rank="AIR ?" compact />}
                    {b.kind === "officer" && <VerifiedOfficerBadge post="?" />}
                  </td>
                  <td className="text-[#3a2e22]">{b.evidence}</td>
                  <td className="num-mono text-[#6C5038]">{b.at}</td>
                  <td className="right">
                    <div className="flex gap-1.5 justify-end">
                      <button className="text-[11px] px-2.5 py-1 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Open evidence</button>
...
window.ScreenAdminCommunity = ScreenAdminCommunity;
