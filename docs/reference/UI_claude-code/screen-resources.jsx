...
function ScreenResources() {
  const [type, setType]   = useStateR("all");
  const [trust, setTrust] = useStateR("all");
  const [exam, setExam]   = useStateR("UPSC CSE");
  ...
  return (
    <div data-screen-label="Resource library">
      <PageHeader eyebrow="Resource library"
      ...
      <div className="px-10 grid grid-cols-[260px_1fr] gap-6">
          ...
          <Card>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2 items-center">
                <span className="num-mono text-[10.5px] text-[#6C5038]">Sort:</span>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {["all","UPSC CSE","SSC CGL","IBPS PO","RBI Grade B"].map(e => (
            <button key={e} onClick={()=>setExam(e)}
              className={`text-[11px] px-2.5 py-1 rounded-full border ${exam===e ? 'bg-[#2E2218] text-[#F3EADB] border-[#2E2218]' : 'border-[#E7DECB] text-[#6C5038]'}`}>{e === "all" ? "All" : e}</button>
          ))}
        </div>
      </Card>
        ...
        <div className="mt-2 flex flex-col gap-1">
          {[
            ...
            <button key={k} onClick={()=>setType(k)}
              className={`text-left text-[12px] px-2.5 py-1.5 rounded-md ${type===k ? 'bg-[#2E2218] text-[#F3EADB]' : 'text-[#3a2e22] hover:bg-[#F3EADB]'}`}>{label}</button>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="mt-2 flex flex-col gap-1.5">
          {[
            ...
            <button key={k} onClick={()=>setTrust(k)}
              className={`flex items-center gap-2 text-left text-[12px] px-2.5 py-1.5 rounded-md ${trust===k ? 'bg-[#2E2218] text-[#F3EADB]' : 'text-[#3a2e22] hover:bg-[#F3EADB]'}`}>
              {k !== "all" && <SourceTrustStamp trust={k} />}
              {k === "all" && <span className="w-2 h-2 bg-[#A68057] rounded-sm"></span>}
              <span>{label}</span>
            </button>
          ))}
        </div>
  ...
  return (
    <article className={`rounded-xl border bg-white/70 p-4 transition hover:border-[#A68057] ${r.flagged ? 'border-[#D9B4A6]' : 'border-[#E7DECB]'}`}>
      <div className="flex items-start gap-3">
        <span className="w-10 h-10 rounded-lg flex items-center justify-center text-[18px] shrink-0"
          style={{ background:"#F3EADB", color:"#6C5038", border:"1px solid #E7DECB" }}>{typeInfo.glyph}</span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            <SourceTrustStamp trust={r.sourceTrust} />
            <Pill tone="outline" className="!text-[9.5px]">{typeInfo.label}</Pill>
            {r.verifiedByTopper && <VerifiedTopperBadge rank="✓ Topper" compact />}
            {r.flagged && <span className="stamp stamp-aggregator" style={{background:'#F2DDD6', color:'#7A3925', border:'1px solid #D9B4A6'}}>Flagged · review</span>}
          </div>
          <h3 className="font-serif text-[16px] mt-2 leading-snug">{r.title}</h3>
          <div className="num-mono text-[10.5px] text-[#6C5038] mt-1.5">
      ...
      <div className="rule mt-3 pt-3 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[11.5px] truncate flex items-center gap-1.5">{u.name}<UserBadge user={u} compact /></div>
            <div className="num-mono text-[10px] text-[#6C5038]">contributed {r.createdAt}</div>
          </div>
        </div>
        <div className="flex items-center gap-3 text-[10.5px] text-[#6C5038] shrink-0">
      ...
      <div className="rule mt-3 pt-2 flex gap-2">
        ...
        <button className="text-[11.5px] px-2.5 py-1.5 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Save</button>
        <button className="text-[11.5px] px-2.5 py-1.5 rounded-full border border-[#E7DECB] text-[#6C5038] font-semibold">Report</button>
      </div>
    </article>
  );
}
    ...
    <Card className="!bg-[#F2DDD6] !border-[#D9B4A6]">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[12.5px] text-[#7A3925]/90 mt-1.5">Flagged resources stay visible but with a clear warning until admin review. After review they're either restored or removed. Resource library moderation lives in <a href="#admincomm" className="underline font-semibold">/admin/community</a>.</p>
        </div>
      </div>
    </Card>
  );
...
window.ScreenResources = ScreenResources;
