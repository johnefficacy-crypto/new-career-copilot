 ...
 *           Prototype map · Trust deep-dive · Exams · Pricing · FAQ · Footer
 * ===================================================================== */
  ...
  return (
    <header className="nav-glass sticky top-0 z-30">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 h-[64px] flex items-center justify-between">
        ...
        <nav className="hide-mb flex items-center gap-7 text-[13.5px] text-[#3a2e22]">
          ...
          <a href="studyos.html" className="link-under text-[#6C5038]">See the prototype →</a>
        </nav>
        <div className="flex items-center gap-2">
          <a href="#" className="btn btn-ghost hide-mb">Sign in</a>
    ...
    <section id="top" className="relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 pt-16 pb-20 grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] gap-12 items-center">
        <div className="relative">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#E7DECB] bg-white/60 text-[12px] text-[#6C5038]">
            <span className="sdot w-2 h-2 rounded-full bg-[#54794E] inline-block pulse-dot"></span>
          ...
          <h1 className="font-serif text-[52px] lg:text-[64px] leading-[1.02] mt-5 text-balance">
            An operating system<br/>for exam prep.
          </h1>
          <p className="text-[17px] text-[#3a2e22] mt-5 max-w-[55ch] leading-relaxed">
            Career Copilot converts <strong>verified exam signals</strong> and your <strong>personal progress</strong> into one concrete plan a day. No content firehose. No motivational fluff. No "AI says so" without showing the work.
          </p>
          <div className="mt-7 flex gap-3 flex-wrap">
            <a href="#start" className="btn btn-primary">Start free · pick your exam</a>
            <a href="studyos.html" className="btn btn-ghost">See a real day → </a>
          </div>
...
function HeroPreview() {
  return (
    <div className="relative">
      <div className="absolute -inset-8 dotted opacity-50 pointer-events-none"></div>
      <div className="relative soft-card grain p-5 lift">
        <div className="flex items-center justify-between">
          <div>
            <div className="eyebrow">Today · Tue, May 14</div>
            <div className="font-serif text-[20px] mt-1.5">Federalism foundations<br/>+ Mock 14 prep</div>
          </div>
          <div className="text-right">
            <div className="num-mono text-[10px] text-[#6C5038]">7 tasks · 6.5h</div>
            <div className="mt-1 inline-flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#54794E] pulse-dot"></span><span className="num-mono text-[10px] text-[#33482F]">live</span></div>
          </div>
        </div>
        ...
        {/* Mini tasks */}
        {[
          { time:"06:30", title:"Polity · Federalism — concept revision", chips:[{l:"user",t:"weak"},{l:"engine",t:"spaced"},{l:"exam",t:"PYQ-heavy"}], done:true },
          { time:"10:00", title:"Modern History · Revolts of 1857 — deep read", chips:[{l:"exam",t:"prereq"},{l:"engine",t:"pre-mock"}], one:true },
          { time:"19:30", title:"Full-length Mock 14 — Prelims P1", chips:[{l:"user",t:"cadence"},{l:"exam",t:"phase"}] },
        ].map((t,i) => (
          <div key={i} className="grid grid-cols-[18px_50px_1fr] gap-3 py-2.5 border-t border-[#EFE7D4] first:border-0">
            <span className={`tick mt-1 ${t.done ? 'done' : ''}`}></span>
            <span className="num-mono text-[11px] text-[#6C5038] pt-1">{t.time}</span>
            <div>
              <div className={`text-[13px] ${t.done ? 'line-through text-[#A68057]' : 'font-medium'}`}>{t.title}</div>
              <div className="mt-1.5 flex flex-wrap gap-1">
                {t.chips.map((c,j) => (
                  <span key={j} className={`chip chip-${c.l}`}>
                    <span style={{opacity:0.6, fontWeight:700}}>{c.l === 'user' ? 'u·' : c.l === 'exam' ? 'e·' : c.l === 'update' ? 'n·' : '⚙'}</span>
                    {c.t}
                  </span>
                ))}
        ...
        <div className="flex items-center justify-between text-[11px]">
          ...
          <a href="studyos.html#today" className="text-[#2E2218] font-semibold link-under">Open today →</a>
        </div>
      </div>

      {/* Floating verified seal callout */}
      <div className="absolute -left-6 lg:-left-10 top-12 soft-card grain p-3 pr-5 flex items-center gap-3 lift" style={{maxWidth:260}}>
        <span className="seal-verified inline-flex items-center justify-center rounded-full" style={{width:28, height:28}}>
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M3 8.4 6.4 11.5 13 4.6" stroke="#F0F5EF" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </span>
        <div>
      ...
      <div className="absolute -right-2 lg:-right-6 bottom-8 soft-card grain px-4 py-3 lift" style={{maxWidth:240}}>
        <div className="eyebrow">Engine</div>
        <div className="text-[12.5px] mt-1">7 rules fired · 4 layers used</div>
        <div className="mt-2 flex gap-1">
          {['u','e','n','⚙'].map((g,i) => (
            <span key={i} className={`chip ${i===0?'chip-user':i===1?'chip-exam':i===2?'chip-update':'chip-engine'}`} style={{padding:'2px 6px'}}>{g}</span>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="border-y border-[#E7DECB] bg-[#FBF8F2] overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-3 flex items-center gap-6">
        <div className="shrink-0 num-mono text-[10.5px] text-[#6C5038] tracking-[0.16em] uppercase whitespace-nowrap">Last 7 days · trust feed</div>
        <div className="overflow-hidden flex-1">
          <div className="marquee">
            {[...items, ...items].map((it, i) => (
              <span key={i} className="inline-flex items-center gap-2 text-[12.5px]">
                <span className={`stamp ${it.kind === 'verified' ? 'stamp-verified' : 'stamp-needs'}`}>{it.kind === 'verified' ? 'Official' : 'Aggregator'}</span>
...
function Pillars() {
  const pillars = [
    {
      k:"01", glyph:"u·", color:"#ECE7F2", border:"#8F86A1",
    ...
    {
      k:"02", glyph:"e·", color:"#E4EDE0", border:"#94B28A",
    ...
    {
      k:"03", glyph:"n·", color:"#F1E1CD", border:"#D6BC93",
      ...
      bullets:["Official · auto-apply", "Aggregator · discovery", "Research · hint only", "Opportunity · adjacent"],
    },
    {
      k:"04", glyph:"⚙",  color:"#2E2218", border:"#2E2218", dark:true,
  ...
  return (
    <section className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
        {pillars.map((p,i) => <PillarCard key={i} p={p} />)}
      </div>
    </section>
  );
}
...
function PillarCard({ p }) {
  return (
    <div className={`relative rounded-2xl border p-6 lift ${p.dark ? 'text-[#F3EADB]' : 'text-[#2E2218]'}`}
         style={{background:p.color, borderColor:p.border}}>
      <div className="flex items-center gap-2">
        <span className={`chip chip-${p.k === '01' ? 'user' : p.k === '02' ? 'exam' : p.k === '03' ? 'update' : 'engine'}`} style={{padding:'4px 8px', fontSize:11}}>{p.glyph}</span>
        <span className={`num-mono text-[10px] ${p.dark ? 'text-[#D6BC93]' : 'text-[#6C5038]'} tracking-[0.16em]`}>{p.k}</span>
      </div>
      <h3 className={`font-serif text-[22px] mt-3 leading-tight ${p.dark ? 'text-[#F3EADB]' : ''}`}>{p.title}</h3>
      <div className={`text-[12.5px] mt-1 ${p.dark ? 'text-[#D6BC93]' : 'text-[#6C5038]'}`}>{p.sub}</div>
      <p className={`text-[13.5px] mt-4 leading-relaxed ${p.dark ? 'text-[#D6BC93]' : 'text-[#3a2e22]'}`}>{p.body}</p>
      <ul className={`mt-5 space-y-1.5 text-[12.5px] ${p.dark ? 'text-[#F3EADB]' : 'text-[#2E2218]'}`}>
        {p.bullets.map((b,i) => (
          <li key={i} className="flex items-center gap-2">
            <span className={`w-1 h-1 rounded-full ${p.dark ? 'bg-[#D6BC93]' : 'bg-[#A68057]'}`}></span>
            <span>{b}</span>
          </li>
        ))}
      </ul>
...
function HowItWorks() {
  const STEPS = [
    ...
    { id:"compile",  title:"Engine compiles",   sub:"Rules fire. Conflicts resolve.", visual: <StepVisualCompile /> },
  ...
  function pick(i) { setActive(i); setProgress(0); }
      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20"
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-8">
          ...
          <div className="space-y-3">
            {STEPS.map((s, i) => (
              <button key={s.id} onClick={()=>pick(i)} className={`step-tab w-full ${active === i ? 'active' : ''}`}>
                <span className="step-num">{String(i+1).padStart(2,'0')}</span>
                <span className="flex-1">
                  <span className={`block font-serif text-[18px] leading-tight ${active === i ? 'text-[#F3EADB]' : 'text-[#2E2218]'}`}>{s.title}</span>
            ...
            <div className="text-[11.5px] text-[#6C5038] pl-1 pt-2">
              Auto-cycling · {paused ? <em>paused</em> : <span>{Math.max(0, Math.round((STEP_MS * (1 - progress/100))/1000))}s to next</span>}
            </div>
          </div>
            ...
            <div className="soft-card grain p-7 min-h-[460px] relative overflow-hidden">
              <div key={active} className="drift-in">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="eyebrow">Step {String(active+1).padStart(2,'0')} · {STEPS[active].id}</div>
                  ...
                  <div className="num-mono text-[10px] text-[#6C5038]">step {active+1}/{STEPS.length}</div>
                </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-6 items-start">
      <ul className="space-y-2">
        {signals.map((s,i) => (
          <li key={i} className="flex items-center gap-3 text-[13px] drift-in" style={{animationDelay: `${s.t}ms`}}>
            <span className={`chip chip-${s.layer}`} style={{minWidth:46, justifyContent:'center'}}>{s.layer === 'user' ? 'u·' : s.layer === 'exam' ? 'e·' : s.layer === 'update' ? 'n·' : '⚙'} {s.layer}</span>
...
function StepVisualCompile() {
  const rules = [
    ...
    { name:"pattern_research",    layer:"update", note:"Polity weightage hint", fired:false, reason:"research-only · no plan change" },
    { name:"aggregator_promote",  layer:"update", note:"Admit-card rumor", fired:false, reason:"awaits official source" },
  ];
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-6">
      <ul className="space-y-2">
        {rules.map((r,i) => (
          <li key={i} className="rounded-lg border border-[#E7DECB] bg-[#FBF8F2] px-3.5 py-2.5 drift-in" style={{animationDelay:`${i*120}ms`}}>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                <span className={`chip chip-${r.layer}`}>{r.layer === 'user' ? 'u·' : r.layer === 'exam' ? 'e·' : r.layer === 'update' ? 'n·' : '⚙'} {r.layer}</span>
                <span className="font-mono text-[12px] truncate">{r.name}</span>
              </div>
              {r.fired ? <span className="stamp stamp-verified">fired</span> : <span className="stamp stamp-needs">held</span>}
            </div>
            <div className="text-[11.5px] text-[#6C5038] mt-1.5 pl-1">{r.note}{r.reason ? ` · ${r.reason}` : ''}</div>
          </li>
        ))}
      </ul>
      <div className="rounded-xl border border-[#2E2218] bg-[#2E2218] text-[#F3EADB] p-4">
        ...
        <div className="font-serif text-[20px] mt-1">7 rules<br/>fired</div>
        <div className="text-[11px] text-[#D6BC93] mt-2">2 held back · trust policy</div>
        <div className="hairline my-3" style={{background:'linear-gradient(90deg,transparent,#4E3A29,transparent)'}}></div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-6">
      <div className="rounded-xl border border-[#E7DECB] bg-white/70 p-3">
        {tasks.map((t,i) => (
          <div key={i} className="grid grid-cols-[18px_50px_1fr_auto] gap-3 py-2 items-center border-t border-[#EFE7D4] first:border-0 drift-in" style={{animationDelay:`${i*120}ms`}}>
...
/* ----------------------------------------------------------------- *
 *  ELIGIBILITY ENGINE
 *  Continuous matching of aspirant profile to verified recruitments
 * ----------------------------------------------------------------- */
    ...
    <section id="eligibility" className="bg-[#FBF8F2] border-y border-[#E7DECB]">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20"
        ...
        <SectionHeader
          ...
          title={<>Matched the moment <em>anything</em> changes.</>}
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-[1fr_460px] gap-8 items-start">
          ...
          <div className="soft-card grain p-7 min-h-[420px] relative overflow-hidden">
            <div className="flex items-center justify-between gap-4">
              <div className="num-mono text-[10.5px] text-[#6C5038] tracking-[0.18em] uppercase">Live scenario · auto-cycling</div>
              <div className="flex items-center gap-1.5">
                {SCENARIOS.map((_,i) => (
                  <button key={i} onClick={()=>setScenario(i)} aria-label={`Scenario ${i+1}`}
                    className={`h-2 rounded-full transition-all ${i === scenario ? 'w-8 bg-[#2E2218]' : 'w-2 bg-[#D6C9AC]'}`}></button>
                ))}
              </div>
            </div>
            ...
            <div key={scenario} className="drift-in mt-4">
              ...
              <div className="mt-6 grid grid-cols-[1fr_60px_1fr] gap-3 items-stretch">
                ...
                <div className="rounded-xl border border-[#8F86A1] bg-[#ECE7F2] p-4">
                  <div className="num-mono text-[9.5px] text-[#31293B] tracking-[0.16em] uppercase">Your profile</div>
                  <ul className="mt-2.5 space-y-1.5">
                    {S.facets.map((f,i) => (
                      <li key={i} className="flex items-center gap-2 text-[12.5px] text-[#31293B] drift-in" style={{animationDelay: `${100 + i*120}ms`}}>
                        <span className="w-1.5 h-1.5 rounded-full bg-[#8F86A1]"></span>
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="flex flex-col items-center justify-center">
                  <div className="relative">
                    <div className="w-12 h-12 rounded-full bg-[#2E2218] flex items-center justify-center text-[#D6BC93] font-mono text-[20px] breathe">⚙</div>
                    <div className="absolute inset-0 rounded-full border-2 border-[#2E2218] opacity-40 breathe" style={{animationDelay:'0.5s'}}></div>
                  </div>
                  <div className="num-mono text-[9px] text-[#6C5038] tracking-[0.16em] uppercase mt-2">6 rules</div>
                </div>
                {/* Verdicts */}
                <div className="rounded-xl border border-[#D6BC93] bg-[#F1E1CD] p-4">
                  <div className="num-mono text-[9.5px] text-[#6C5038] tracking-[0.16em] uppercase">Verdicts</div>
                  <ul className="mt-2.5 space-y-1.5">
                    {S.flips.map((f,i) => (
                      <li key={i} className="flex items-start gap-2 text-[12px] drift-in" style={{animationDelay:`${600 + i*180}ms`}}>
                        <VerdictDot v={f.to} />
                        <span className="flex-1">
              ...
              <div className="hairline my-5"></div>
              <div className="flex items-center justify-between">
                <div className="num-mono text-[11px] text-[#6C5038]">delta: <span className="text-[#33482F]">{S.delta}</span></div>
                <a href="studyos.html#elig" className="text-[12px] font-semibold text-[#2E2218] link-under">Open eligibility matches →</a>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-2xl border border-[#94B28A] bg-[#F0F5EF] p-5">
              <div className="flex items-start gap-3">
                <span className="seal-verified inline-flex items-center justify-center rounded-full shrink-0" style={{width:28, height:28}}>
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M3 8.4 6.4 11.5 13 4.6" stroke="#F0F5EF" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </span>
                <div>
                  <div className="num-mono text-[9.5px] text-[#33482F] tracking-[0.18em]">TRUST POLICY</div>
                  <div className="font-serif text-[16px] mt-1 leading-snug text-[#33482F]">A criterion is scraped first, then reviewer-locked before it can flip a verdict.</div>
                  <p className="text-[11.5px] text-[#33482F] mt-2">Until verified, we mark your verdict <em>conditional</em>. We never silently say "eligible" based on aggregator data.</p>
                </div>
              </div>
            </div>
            <div className="rounded-2xl border border-[#2E2218] bg-[#2E2218] text-[#F3EADB] p-5">
              ...
              <div className="font-serif text-[16px] mt-1 leading-snug">Re-matches happen every 15m for scraped sources, instantly when you edit your profile.</div>
...
function VerdictDot({ v }) {
  ...
  return (
    <span className="mt-1 w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 text-[#F0F5EF] text-[9px] font-bold" style={{background:m.c}}>{m.text}</span>
  );
}
...
function FactTile({ k, v, sub, tone }) {
  const dark = tone === "ink";
  return (
    <div className={`rounded-xl border p-4 ${dark ? 'bg-[#2E2218] border-[#2E2218]' : tone === 'sage' ? 'bg-[#F0F5EF] border-[#B9CFAF]' : 'bg-white/70 border-[#E7DECB]'}`}>
      <div className={`num-mono text-[9.5px] tracking-[0.16em] uppercase ${dark ? 'text-[#D6BC93]' : 'text-[#6C5038]'}`}>{k}</div>
      <div className={`font-serif text-[28px] leading-none mt-1.5 ${dark ? 'text-[#F3EADB]' : tone === 'sage' ? 'text-[#33482F]' : 'text-[#2E2218]'}`}>{v}</div>
      <div className={`text-[10.5px] mt-2 leading-snug ${dark ? 'text-[#D6BC93]' : 'text-[#6C5038]'}`}>{sub}</div>
    </div>
  );
}
...
function ExploreScreens() {
  const screens = [
    { tag:"Aspirant", title:"Today · Mission Control",   sub:"Plan + reasoning trace per task",        href:"studyos.html#today",    glyph:"◐", tone:"clay" },
    { tag:"Aspirant", title:"Study Plan",                sub:"Timeline + before/after adaptation",     href:"studyos.html#plan",     glyph:"▤", tone:"sage" },
    { tag:"Aspirant", title:"Subjects",                  sub:"Topic tree · locked priority · mastery", href:"studyos.html#subjects", glyph:"❖", tone:"dusk" },
    { tag:"Aspirant", title:"Focus",                     sub:"25/50/90m timer · reflection drawer",    href:"studyos.html#focus",    glyph:"◍", tone:"clay" },
    { tag:"Aspirant", title:"Mocks",                     sub:"Error patterns → correction tasks",      href:"studyos.html#mocks",    glyph:"△", tone:"sage" },
    { tag:"Aspirant", title:"Weekly review",             sub:"Honest read · next-week preview",        href:"studyos.html#review",   glyph:"↻", tone:"dusk" },
    { tag:"Aspirant", title:"Eligibility matches",       sub:"Recruitments matched to your profile",   href:"studyos.html#elig",     glyph:"⌖", tone:"clay" },
    { tag:"Admin",    title:"Exam Intelligence",         sub:"7 tabs · verify + lock + plan-impact",   href:"studyos.html#admexam",  glyph:"⊞", tone:"clay", dark:true },
    { tag:"Admin",    title:"Eligibility verification",  sub:"Criteria queue · match impact preview",  href:"studyos.html#admelig",  glyph:"⌗", tone:"sage", dark:true },
    { tag:"Admin",    title:"Persona Inspector",         sub:"Internal dims → study policy",           href:"studyos.html#admper",   glyph:"◊", tone:"dusk", dark:true },
  ];
  return (
    <section className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20">
      <SectionHeader
        ...
        sub="Ten live screens — seven for aspirants, three for admins. Every panel is labelled live / preview / not connected." />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mt-10">
        {screens.map((s,i) => <ScreenLinkCard key={i} s={s} />)}
      </div>
      <div className="mt-6 text-center">
        <a href="studyos.html#handoff" className="btn btn-ghost">Or read the handoff + gap list →</a>
      </div>
    </section>
  );
}
...
function ScreenLinkCard({ s }) {
  return (
    <a href={s.href} className={`group relative rounded-2xl border p-5 lift ${s.dark ? 'bg-[#2E2218] border-[#2E2218]' : 'bg-white/70 border-[#E7DECB]'}`}>
      <div className="flex items-center justify-between">
        <span className={`text-[10.5px] font-semibold tracking-[0.18em] uppercase ${s.dark ? 'text-[#D6BC93]' : 'text-[#6C5038]'}`}>{s.tag}</span>
        <span className={`text-[20px] ${s.dark ? 'text-[#D6BC93]' : 'text-[#A68057]'}`}>{s.glyph}</span>
      </div>
      <div className={`font-serif text-[18px] mt-3 leading-tight ${s.dark ? 'text-[#F3EADB]' : 'text-[#2E2218]'}`}>{s.title}</div>
      <div className={`text-[12.5px] mt-1.5 ${s.dark ? 'text-[#D6BC93]' : 'text-[#6C5038]'}`}>{s.sub}</div>
      <div className={`mt-5 text-[12px] font-semibold flex items-center gap-1.5 ${s.dark ? 'text-[#F3EADB]' : 'text-[#2E2218]'}`}>
        <span>Open</span>
    ...
    <section id="trust" className="bg-[#FBF8F2] border-y border-[#E7DECB]">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20">
        <SectionHeader
          ...
          title={<>We never silently treat <em>rumour</em> as <em>news</em>.</>}
          sub="Two updates can look the same. We hold them very, very far apart in how they affect your plan."/>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-10">
          {/* Verified card */}
          <div className="relative rounded-2xl border border-[#94B28A] bg-gradient-to-b from-[#DAE7D6]/40 to-[#F8FAF6] p-6">
            <div className="flex items-start gap-3">
              <span className="seal-verified inline-flex items-center justify-center rounded-full" style={{width:36, height:36}}>
                <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><path d="M3 8.4 6.4 11.5 13 4.6" stroke="#F0F5EF" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </span>
              <div>
                <span className="stamp stamp-verified">Officially verified</span>
                <h3 className="font-serif text-[22px] mt-2 leading-tight">UPSC CSE 2026 — application opens May 22</h3>
                <p className="text-[13px] text-[#33482F] mt-2 max-w-[52ch]">Sourced from upsc.gov.in · cross-checked by admin curator · cryptographically signed before reaching aspirants.</p>
              </div>
            </div>
            ...
            <div className="mt-4 num-mono text-[10px] text-[#33482F]">sig 0x4f·a7c2 · received May 12 · 09:14 IST</div>
          </div>

          {/* Aggregator card */}
          <div className="rounded-2xl p-6" style={{border:'1px dashed #8F86A1', background:'repeating-linear-gradient(135deg, rgba(143,134,161,0.045) 0 8px, transparent 8px 16px), #FBF8F2'}}>
            <div className="flex items-start gap-3">
              <span className="inline-flex items-center justify-center rounded-full" style={{width:36, height:36, background:'#FBF8F2', border:'1px dashed #8F86A1', color:'#524864'}}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6.5" stroke="#524864" strokeWidth="1.2" strokeDasharray="2 2"/><path d="M8 4.5v4.2M8 11.2v.6" stroke="#524864" strokeWidth="1.6" strokeLinecap="round"/></svg>
              </span>
              <div>
                <span className="stamp stamp-needs">Aggregator · needs verification</span>
                <h3 className="font-serif text-[22px] mt-2 leading-tight text-[#31293B]">Admit card likely by Jul 28 (rumoured)</h3>
                <p className="text-[13px] text-[#31293B] mt-2 max-w-[52ch]">Reported by exam-news aggregators based on staffing notices. No official communication yet. We hold it.</p>
              </div>
            </div>
            <div className="mt-4">
              ...
              <div className="mt-1 h-[5px] bg-[#E3DFEA] rounded-full overflow-hidden">
                <div className="h-full bg-[#B7B0C4]" style={{width:'42%'}}></div>
              </div>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3 text-[11.5px]">
              <Fact k="What it does" v="Sits in your update feed · admin can promote on confirmation" muted />
              <Fact k="What it doesn't do" v="It doesn't move your dates · it doesn't add tasks" muted />
            </div>
            <div className="mt-4 num-mono text-[10px] text-[#524864]">received May 13 · 18:02 · awaits upsc.gov.in</div>
          </div>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-[#E7DECB] bg-white/70 p-5">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            <PolicyTile stamp="Official"       tone="verified" body="Auto-apply after admin review. May change plan, dates, syllabus." />
            <PolicyTile stamp="Aggregator"     tone="needs"    body="Surface only. Cannot affect your plan until paired with an official source." />
            <PolicyTile stamp="Research"       tone="needs"    body="Strategy hint only. Never auto-edits the plan; only seen in 'why this plan'." />
            <PolicyTile stamp="Opportunity"    tone="verified" body="Adjacent exams matched to you. Separate from your active prep — never silent." />
          </div>
        </div>
      </div>
    </section>
...
function Fact({ k, v, muted }) {
  return (
    <div className={`rounded-lg border ${muted ? 'border-[#DDDAE3] bg-white/40' : 'border-[#B9CFAF] bg-white/60'} p-3`}>
      <div className="eyebrow !text-[9.5px]" style={muted ? {color:'#524864'} : {color:'#33482F'}}>{k}</div>
...
function ExamsRow() {
  const families = [
    { fam:"Civil Services", e:[ {m:"CS", n:"UPSC CSE", cycle:"2026", state:"connected"}, {m:"PS", n:"UPSC CDS", cycle:"2026·1", state:"connected"}, {m:"FS", n:"IFoS", cycle:"2026", state:"partial"} ] },
    { fam:"Banking",        e:[ {m:"RB", n:"RBI Grade B", cycle:"2026", state:"connected"}, {m:"IB", n:"IBPS PO", cycle:"2026", state:"connected"}, {m:"SB", n:"SBI PO", cycle:"2026", state:"partial"} ] },
    { fam:"SSC",            e:[ {m:"CG", n:"SSC CGL", cycle:"2026", state:"connected"}, {m:"CH", n:"SSC CHSL", cycle:"2026", state:"connected"}, {m:"MT", n:"SSC MTS", cycle:"2026", state:"partial"} ] },
    { fam:"State PSC",      e:[ {m:"UP", n:"UPPSC", cycle:"2026", state:"partial"}, {m:"BR", n:"BPSC", cycle:"2026", state:"partial"}, {m:"MH", n:"MPSC", cycle:"2026", state:"partial"} ] },
    { fam:"Defense · others", e:[ {m:"NA", n:"NDA", cycle:"2026·1", state:"connected"}, {m:"AF", n:"AFCAT", cycle:"2026", state:"partial"}, {m:"RR", n:"RRB NTPC", cycle:"2026", state:"partial"} ] },
  ];
  return (
    <section id="exams" className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20">
      ...
      <div className="space-y-3 mt-10">
        {families.map((f,i) => (
          <div key={i} className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-4 items-start border-t border-[#E7DECB] pt-5">
            <div>
              <div className="font-serif text-[18px]">{f.fam}</div>
              <div className="num-mono text-[10.5px] text-[#6C5038] mt-1 tracking-[0.1em]">{f.e.length} exams</div>
            </div>
            <div className="flex flex-wrap gap-2.5">
              {f.e.map((x,j) => (
                <div key={j} className="exam-chip text-[13px]">
                  ...
                  <span className={`stamp ${x.state === 'connected' ? 'stamp-verified' : 'stamp-needs'}`} style={{padding:'1px 5px'}}>{x.state === 'connected' ? 'connected' : 'partial'}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-10">
          <PricingCard
            ...
            price="Free"
            note="forever"
            bullets={["Today · Mission Control","Subjects + topic tree","Weekly review (manual)","Update feed (read-only)"]}
            cta="Start free"
          />
          <PricingCard
            tier="Study OS"
            ...
            highlight
            bullets={["Engine compiles your day","Plan adaptation + change log","Mock correction tasks","Spaced revision","Trust-graded updates · auto-applied"]}
          ...
          <PricingCard
            ...
            bullets={["Everything in Study OS","2× mentor review sessions / mo","Personalized policy tuning","Priority verified-update channel"]}
            cta="Talk to mentor"
          />
        </div>
        <div className="text-center mt-5 text-[12.5px] text-[#6C5038]">Govt-employee parents · serving students · NCC: 30% off, pay what helps · no questions asked.</div>
      </div>
    </section>
  );
}
...
function PricingCard({ tier, price, note, bullets, cta, highlight }) {
  return (
    <div className={`rounded-2xl border p-6 ${highlight ? 'bg-[#2E2218] border-[#2E2218] text-[#F3EADB] lift' : 'bg-white/70 border-[#E7DECB]'}`}>
      <div className={`eyebrow ${highlight ? '!text-[#D6BC93]' : ''}`}>{tier}</div>
      <div className="mt-3 flex items-baseline gap-2">
        <span className={`font-serif text-[40px] leading-none ${highlight ? 'text-[#F3EADB]' : ''}`}>{price}</span>
      ...
      <ul className={`mt-5 space-y-2 text-[13px] ${highlight ? 'text-[#D6BC93]' : 'text-[#3a2e22]'}`}>
        {bullets.map((b,i) => (
          <li key={i} className="flex items-start gap-2.5">
            <span className={`mt-0.5 w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${highlight ? 'bg-[#D6BC93]' : 'bg-[#54794E]'}`}>
              <svg width="9" height="9" viewBox="0 0 16 16" fill="none"><path d="M3 8.4 6.4 11.5 13 4.6" stroke={highlight ? '#2E2218' : '#F0F5EF'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </span>
            <span>{b}</span>
          </li>
        ))}
      </ul>
      <a href="#" className={`mt-6 btn w-full justify-center ${highlight ? 'bg-[#F3EADB] text-[#2E2218]' : 'btn-primary'}`}>{cta}</a>
    </div>
  );
}
    ...
    <section id="start" className="max-w-[1280px] mx-auto px-6 lg:px-10 py-20">
      <div className="relative soft-card grain p-12 lg:p-16 overflow-hidden text-center">
        ...
        <h2 className="font-serif text-[44px] lg:text-[56px] leading-[1.05] mt-3 max-w-[18ch] mx-auto">
          One compiled day.<br/>Every morning.
        </h2>
        ...
        <div className="mt-8 flex justify-center gap-3 flex-wrap">
          <a href="#" className="btn btn-primary">Start free · pick your exam</a>
          <a href="studyos.html" className="btn btn-ghost">Walk through the prototype</a>
        </div>
        <div className="mt-6 text-[11.5px] text-[#6C5038]">No credit card. 7-day Study OS trial included.</div>
      </div>
    </section>
  );
}
    ...
    <footer className="border-t border-[#E7DECB] bg-[#FBF8F2]">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-12 grid grid-cols-2 md:grid-cols-5 gap-8">
        <div className="col-span-2">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#2E2218] flex items-center justify-center"><span className="font-serif text-[15px] text-[#F3EADB] leading-none">cc</span></div>
            <div className="font-serif text-[16px]">Career Copilot</div>
          </div>
        ...
        <FooterCol h="Prototype" links={[
          {l:"Today · Mission Control", h:"studyos.html#today"},
          {l:"Study Plan", h:"studyos.html#plan"},
          {l:"Subjects", h:"studyos.html#subjects"},
          {l:"Eligibility matches", h:"studyos.html#elig"},
          {l:"Weekly review", h:"studyos.html#review"},
          {l:"Admin · Exam Intelligence", h:"studyos.html#admexam"},
          {l:"Admin · Eligibility", h:"studyos.html#admelig"},
          {l:"Handoff & gaps", h:"studyos.html#handoff"},
        ]} />
      ...
      <div className="border-t border-[#E7DECB]">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-10 py-5 flex flex-wrap items-center justify-between gap-3">
          <div className="num-mono text-[10.5px] text-[#6C5038]">© 2026 Career Copilot · ccp-mainbuild-v1 · study-os v0.6</div>
          <div className="flex items-center gap-3 num-mono text-[10.5px] text-[#6C5038]">
            <span className="inline-flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#54794E]"></span>all systems live</span>
            <span>·</span>
            <a href="#" className="link-under">Privacy</a>
            <a href="#" className="link-under">Terms</a>
            <a href="#" className="link-under">Status</a>
          </div>
        </div>
      </div>
    </footer>
  ...
  return (
    <div>
      ...
      <ul className="mt-3 space-y-2">
        {links.map((l,i) => <li key={i}><a href={l.h} className="text-[13px] text-[#3a2e22] link-under">{l.l}</a></li>)}
      </ul>
    </div>
  );
}
