/* global React, ReactDOM, ScreenToday, ScreenPlan, ScreenSubjects, ScreenFocus, ScreenMocks, ScreenReview, ScreenAdminExam, ScreenAdminPersona, ScreenHandoff, TweaksPanel, TweakSection, TweakRadio, TweakToggle, useTweaks */
const { useState: useStateApp, useEffect: useEffectApp } = React;

const ROUTES = [
  { id:"today",    path:"/app/today",                title:"Today",          group:"aspirant", desc:"Mission Control",         render: () => <ScreenToday /> },
  { id:"plan",     path:"/app/study-plan",           title:"Study Plan",     group:"aspirant", desc:"Timeline · adaptation",   render: () => <ScreenPlan /> },
  { id:"subjects", path:"/app/study/subjects",       title:"Subjects",       group:"aspirant", desc:"Topic intelligence",      render: () => <ScreenSubjects /> },
  { id:"focus",    path:"/app/study/focus",          title:"Focus",          group:"aspirant", desc:"Timed session",           render: () => <ScreenFocus /> },
  { id:"mocks",    path:"/app/study/mocks",          title:"Mocks",          group:"aspirant", desc:"Analysis · correction",   render: () => <ScreenMocks /> },
  { id:"review",   path:"/app/study/review",         title:"Weekly review",  group:"aspirant", desc:"Close the loop",          render: () => <ScreenReview /> },
...
function Sidebar({ route, onPick }) {
  const aspirant = ROUTES.filter(r => r.group === 'aspirant');
  const admin    = ROUTES.filter(r => r.group === 'admin');
  const meta     = ROUTES.filter(r => r.group === 'meta');
  return (
    <aside className="w-[248px] shrink-0 border-r border-[#E7DECB] bg-[#FBF4E8] sticky top-0 h-screen overflow-auto">
      ...
      <nav className="px-3 py-2">
        <div className="nav-section">Aspirant</div>
        {aspirant.map(r => <NavLink key={r.id} r={r} active={r.id === route.id} onPick={onPick} />)}
        <div className="nav-section">Admin · internal</div>
        {admin.map(r => <NavLink key={r.id} r={r} active={r.id === route.id} onPick={onPick} />)}
        <div className="nav-section">Prototype</div>
        {meta.map(r => <NavLink key={r.id} r={r} active={r.id === route.id} onPick={onPick} />)}
      </nav>

      <div className="px-4 py-3 mt-auto border-t border-[#E7DECB]">
